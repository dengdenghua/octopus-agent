 ...
 *   1. env OCTOPUS_GODOT_BIN          (explicit, dev/preview)
 ...
 *   - env OCTOPUS_PET_PROJECT, else <repo>/pet-sidecar
 *
 * Set OCTOPUS_PET_DISABLED=1 to hard-disable pet launch.
 */
...
function petProjectPath() {
  if (process.env.OCTOPUS_PET_PROJECT) return process.env.OCTOPUS_PET_PROJECT;
...
function resolveGodot() {
  if (process.env.OCTOPUS_GODOT_BIN) return process.env.OCTOPUS_GODOT_BIN;
  if (appIsPackaged()) {
    // Packaged: the built sidecar executable shipped inside resources.
    const dir = packagedPetDir();
    const candidates = {
      darwin: ["OctopusPet", "OctopusPet.app/Contents/MacOS/OctopusPet"],
      win32: ["OctopusPet.exe"],
      linux: ["OctopusPet", "octopus-pet"],
    };
