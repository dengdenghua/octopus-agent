...
beforeEach(() => {
  tempRoot = mkdtempSync(path.join(tmpdir(), "octopus-protocol-"));
  distRoot = path.join(tempRoot, "dist");
  mkdirSync(path.join(distRoot, "community"), { recursive: true });
  writeFileSync(path.join(distRoot, "index.html"), "<main>Octopus</main>");
  writeFileSync(
...
describe("desktop renderer origin", () => {
  it("accepts only the fixed app scheme and host", () => {
    expect(
      desktopProtocol.isDesktopAppURL("octopus-app://app/index.html"),
    ).toBe(true);
    expect(
      desktopProtocol.isDesktopAppURL("octopus-app://evil/index.html"),
    ).toBe(false);
...
describe("desktop protocol handler", () => {
  ...
  it("serves absolute community paths from the packaged renderer", async () => {
    ...
    const response = await makeHandler(fetchImpl)(
      new Request("octopus-app://app/community/memory-video(1).jpg"),
    );
  ...
  it("proxies native /api and /api/plugins URLs only to loopback", async () => {
    const fetchImpl = vi.fn(async () =>
      Response.json({ ok: true }, { headers: { "X-Backend": "octopus" } }),
    );
    const handler = makeHandler(fetchImpl);

    const response = await handler(
      new Request(
        "octopus-app://app/api/plugins/paper-trading/page?view=compact",
        {
          headers: {
            Authorization: "Bearer renderer-token",
            Origin: "octopus-app://app",
            Referer: "octopus-app://app/index.html",
          },
        },
      ),
    );
  ...
  it("does not mistake lookalike paths or foreign hosts for backend routes", async () => {
    ...
    const lookalike = await handler(
      new Request("octopus-app://app/api.evil/health"),
    );
    const foreign = await handler(new Request("octopus-app://evil/api/health"));
  ...
  it("streams non-GET request bodies through the loopback proxy", async () => {
    ...
    const response = await makeHandler(fetchImpl)(
      new Request("octopus-app://app/api/test-body", {
        method: "POST",
  ...
  it("rewrites backend redirects back onto the renderer origin", async () => {
    ...
    const response = await makeHandler(fetchImpl)(
      new Request("octopus-app://app/api/health"),
    );

    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe(
      "octopus-app://app/api/health/",
    );
  });
});
