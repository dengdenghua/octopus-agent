...
import type { ChannelName, PairingRequest, PairingStatus } from "@/core/channels/api";
import { cn } from "@/lib/utils";

const CHANNEL_NAMES: Record<string, string> = {
  dingtalk: "DingTalk", wechat: "WeChat", feishu: "Feishu",
  telegram: "Telegram", slack: "Slack", wecom: "WeCom", discord: "Discord",
};

const ALL_CHANNELS: ChannelName[] = ["dingtalk", "wechat", "feishu", "telegram", "slack", "wecom"];
...
function PairingRequestRow({
  ...
  return (
    <div className="workspace-panel-subtle group flex items-center gap-3 rounded-lg px-4 py-3 transition-all duration-200">
      {req.user_avatar ? (
        <img src={req.user_avatar} alt="" className="size-9 rounded-lg object-cover ring-1 ring-border/50" />
      ) : (
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {req.group_name && (
            <span className="text-muted-foreground text-xs">· {req.group_name}</span>
          )}
        </div>
...
export default function PairingPage() {
  ...
  const approveMutation = useMutation({
    ...
    onError: () => { setActionLoading(null); showToast(t.pairing.operationFailed, false); },
  });
      ...
      <WorkspaceBody>
        <div className="mx-auto flex w-full max-w-(--container-width-md) flex-col gap-4 py-2">
          ...
          <section className="workspace-panel rounded-[1.75rem] px-5 py-5 md:px-6">
            ...
            <p className="text-muted-foreground mt-1 text-sm">{t.pairing.description}</p>
          </section>
          ...
          {notImplemented && (
            <div className="workspace-panel rounded-[1.75rem] px-5 py-10 md:px-6">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="text-base font-semibold">{t.pairing.notImplementedTitle}</div>
          ...
          <div className="workspace-panel rounded-[1.75rem] px-5 py-4 md:px-6">
            <div className="flex flex-wrap items-center gap-2">
              {hasFilters && (
                <span className="text-muted-foreground text-xs">{t.pairing.currentFilters}:</span>
              )}
              {filterChannel && (
                <FilterTag
              ...
              <div className="ml-auto flex items-center gap-2">
                <select
                  value={filterChannel}
                  onChange={(e) => setFilterChannel(e.target.value as ChannelName | "")}
                ...
                <select
                  value={filterStatus}
                  onChange={(e) => setFilterStatus(e.target.value as PairingStatus | "")}
                  className="rounded-lg border border-border bg-background px-2 py-1 text-xs outline-none focus:border-primary"
                >
                  <option value="">{t.pairing.allStatuses}</option>
                  <option value="pending">{t.pairing.statusPending}</option>
                  <option value="approved">{t.pairing.statusApproved}</option>
                  <option value="rejected">{t.pairing.statusRejected}</option>
                </select>
                {hasFilters && (
                  <button
          ...
          <div className="workspace-panel rounded-[1.75rem] px-5 py-5 md:px-6">
            <div className="mb-1 text-sm font-semibold">{t.pairing.pendingSection}</div>
            <div className="text-muted-foreground mb-4 text-xs">{t.pairing.pendingNote}</div>
            ...
            {!isLoading && pendingRequests.length === 0 && (
              <div className="flex flex-col items-center py-10">
                <div className="text-muted-foreground/40 mb-2 text-4xl">📭</div>
                <div className="text-sm font-medium">{t.pairing.noPendingRequests}</div>
                <div className="text-muted-foreground mt-1 text-xs">{t.pairing.noPendingDesc}</div>
              </div>
            )}
