        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            <input
              ...
              type="file"
              accept="application/zip,.zip,.octopus-assets"
              hidden
              onChange={(event) => {
...
export default function DesignPage({
  ...
  const revealNodeAsset = useCallback(async (node: DesignCanvasNode) => {
    if (!node.asset?.path || !window.octopus?.desktop) return;
    const result = await window.octopus.desktop.openItem(node.asset.path);
