...
vi.mock("react-router-dom", async () => {
  const actual = await vi.importActual<typeof import("react-router-dom") // eslint-disable-line @typescript-eslint/consistent-type-imports
  >("react-router-dom");
  return {
    ...actual,
...
vi.mock("@/providers/AuthProvider", () => ({
  useAuth: () => ({
    register: (payload: { username: string; password: string; email?: string }) => registerMock(payload),
...
describe("RegisterPage", () => {
  ...
  it("renders the registration form", () => {
    ...
    expect(screen.getByRole("textbox", { name: "邮箱（可选）" })).toBeInTheDocument();
  ...
  it("submits successfully and navigates to login", async () => {
    ...
    await user.type(screen.getByRole("textbox", { name: "用户名" }), "alice");
    await user.type(screen.getByRole("textbox", { name: "邮箱（可选）" }), "alice@example.com");
    await user.type(screen.getByLabelText("密码"), "secret123");
    await user.type(screen.getByLabelText("确认密码"), "secret123");
    await user.click(screen.getByRole("button", { name: "创建账户" }));
