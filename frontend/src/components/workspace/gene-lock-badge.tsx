...
async function geneLockRequest(
  url: string,
  init?: RequestInit,
): Promise<Response> {
  ...
  if (!response.ok)
    throw new Error(`Governance request: HTTP ${response.status}`);
  return response;
}
...
export function GeneLockBadge() {
  ...
  const reload = useCallback(async () => {
    try {
      const r: LockStatus = await geneLockRequest(
        `${getBackendBaseURL()}/api/gene-locks/status`,
      ).then(async (response) => {
        if (!response.ok)
          throw new Error("Governance status: " + response.status);
        const body = await response.json();
        if (
          typeof body?.panic?.active !== "boolean" ||
          !Number.isFinite(body?.maturity_level)
        )
          throw new Error("Governance status is unavailable");
        return body;
      });
      setStatus(r);
    } catch (e) {
  ...
  const changeLevel = useCallback(
    async (delta: number) => {
      ...
      try {
        ...
        setMsg(serviceErrorMessage(error));
      } finally {
        setBusy(false);
  ...
  const triggerPanic = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await geneLockRequest(`${getBackendBaseURL()}/api/gene-locks/panic`, {
        method: "POST",
  ...
  const clearPanic = useCallback(async () => {
    setBusy(true);
    try {
      await geneLockRequest(
        `${getBackendBaseURL()}/api/gene-locks/panic/clear`,
        {
          method: "POST",
...
export function GeneLockControlCard({
  ...
  const setMode = useCallback(
    (mode: "dev" | "production") =>
      run(`mode:${mode}`, () =>
        geneLockRequest(`${getBackendBaseURL()}/api/gene-locks/mode`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "X-Human-Approver": "ui-operator",
  ...
  const setLevel = useCallback(
    (level: number) =>
      run(`level:${level}`, () =>
        geneLockRequest(`${getBackendBaseURL()}/api/gene-locks/maturity`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "X-Human-Approver": "ui-operator",
