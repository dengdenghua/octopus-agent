...
// conservative: requires an action-intro word + a concrete verb.
const OUTWARD_FACING_THINKING_RE =
  /(?:我将|我会|接下来|下面|然后|先|首先|准备|接着|开始)\s*.{0,24}(?:检查|查看|梳理|确认|读取|执行|分析|排查|定位|验证|测试|搜索|了解|修复|创建|删除|修改|运行|构建|部署|浏览|打开|对比|核对)/;
