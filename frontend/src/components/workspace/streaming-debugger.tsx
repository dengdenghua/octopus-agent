 ...
 * - Bug 定位时间从 2h → 30min
 */
export function StreamingDebugger({ events, className }: StreamingDebuggerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [filter, setFilter] = useState("");
  const [selectedEvent, setSelectedEvent] = useState<LiveToolEvent | null>(null);
  const [filterType, setFilterType] = useState<"all" | "error" | "running">("all");
  ...
  return (
    <>
      ...
      <button
        ...
        className={cn(
          "fixed bottom-4 right-4 z-50 flex size-12 items-center justify-center rounded-full border-2 border-primary bg-background shadow-lg transition-all hover:scale-110",
      ...
      {/* 调试面板 */}
      {isOpen && (
        <div className="fixed inset-4 z-50 flex flex-col overflow-hidden rounded-lg border border-border-default bg-background shadow-2xl">
          {/* 头部 */}
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      event.status === "done" && "bg-green-500/10 text-green-500",
              ...
              {selectedEvent ? (
                <>
                  <div className="shrink-0 border-b border-border-subtle bg-muted/30 px-4 py-3">
                    <h4 className="text-sm font-semibold">{selectedEvent.name}</h4>
