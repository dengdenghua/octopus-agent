const BRIDGE_TOKEN_MARKER = "__OCTOPUS_PRIVATE_BRIDGE_TOKEN__";
...
(function() {
  ...
  function postToParent(message) {
    try {
      window.parent.postMessage(
        BRIDGE_TOKEN ? Object.assign({}, message, { octopusBridgeToken: BRIDGE_TOKEN }) : message,
        '*'
      );
    } catch (_) {}
  }
  ...
  function onClick(e) {
    ...
    postToParent({ type: 'octopus:inspect:select', payload: payload });
    setActive(false);
  }
  ...
  function setActive(v) {
    ...
    postToParent({ type: 'octopus:inspect:state', active: active });
  }
  ...
  function announceEditState() {
    postToParent({ type: 'octopus:edit:state', active: editing, dirty: editDirty });
  }
  ...
  function emitEditedContent() {
    ...
    postToParent({ type: 'octopus:edit:content', bodyHtml: bodyHtml });
  }

  window.addEventListener('message', function(e) {
    const data = e && e.data;
    if (!data || typeof data !== 'object') return;
    if (BRIDGE_TOKEN && data.octopusBridgeToken !== BRIDGE_TOKEN) return;
    ...
    else if (data.type === 'octopus:edit:enable') enableEditing();
    else if (data.type === 'octopus:edit:request-save') emitEditedContent();
    else if (data.type === 'octopus:edit:cancel') finishEditing(true);
    else if (data.type === 'octopus:edit:commit') finishEditing(false);
  });

  function announce() {
    postToParent({ type: 'octopus:inspect:ready' });
  }
