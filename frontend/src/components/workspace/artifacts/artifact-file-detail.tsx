...
export function OfficePreview({
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "octopus:office:ready") {
        setIframeReady(true);
        if (selectionRequested) {
          iframeRef.current?.contentWindow?.postMessage(
            { type: "octopus:office:enable" },
            "*",
          );
          setSelecting(true);
          setSelectionRequested(false);
        }
      } else if (data.type === "octopus:office:state") {
        setSelecting(data.active === true);
      } else if (data.type === "octopus:office:select") {
  ...
  const toggleSelection = useCallback(() => {
    ...
    iframeRef.current?.contentWindow?.postMessage(
      { type: next ? "octopus:office:enable" : "octopus:office:disable" },
      "*",
    );
    setSelecting(next);
    if (next) {
  ...
  useEffect(() => {
    ...
    void fetch(previewUrl, {
      ...
      .then(async (response) => {
        ...
        const previewMode = response.headers.get("X-Octopus-Office-Preview");
...
export function HtmlPreview({
  ...
  const postBridgeMessage = useCallback(
    (type: string) => {
      iframeRef.current?.contentWindow?.postMessage(
        { type, octopusBridgeToken: bridgeToken },
        "*",
      );
    },
    [bridgeToken],
  ...
  const persistHumanEdit = useCallback(
    async (bodyHtml: string) => {
      ...
      try {
        ...
        postBridgeMessage("octopus:edit:commit");
        setHumanEditing(false);
        setHumanDirty(false);
        setPendingHumanEdit(false);
        setHumanConflict(null);
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      const data = event.data as {
        ...
        dirty?: unknown;
        bodyHtml?: unknown;
        octopusBridgeToken?: unknown;
      } | null;
      if (!data || typeof data !== "object") return;
      if (data.octopusBridgeToken !== bridgeToken) return;
      if (data.type === "octopus:inspect:ready") {
        setBridgeReady(true);
        if (pendingHumanEdit) {
          postBridgeMessage("octopus:edit:enable");
          setPendingHumanEdit(false);
        }
      } else if (data.type === "octopus:edit:state") {
        setHumanEditing(data.active === true);
        setHumanDirty(data.dirty === true);
      } else if (
        data.type === "octopus:edit:content" &&
  ...
  const cancelHumanEdit = useCallback(() => {
    postBridgeMessage("octopus:edit:cancel");
    setHumanEditing(false);
    setHumanDirty(false);
    setPendingHumanEdit(false);
    setHumanConflict(null);
          ...
          {humanEditing ? (
            <>
              ...
              <Button
                ...
                onClick={() => postBridgeMessage("octopus:edit:request-save")}
                size="sm"
                type="button"
              >
                {humanSaving ? (
