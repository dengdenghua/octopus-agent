...
export async function getDesktopAutomationPermissions(): Promise<DesktopAutomationPermissions> {
  if (!window.octopus?.desktop?.getAutomationPermissions) {
    return {
      supported: false,
      platform: "web",
  ...
  return window.octopus.desktop.getAutomationPermissions();
}
...
export async function openDesktopAutomationPermission(
  ...
  const result =
    await window.octopus?.desktop?.openAutomationPermission?.(permission);
