...
describe("pending-new-session hand-off", () => {
  ...
  it("round-trips a written hand-off and clears it on consume", () => {
    ...
    expect(window.sessionStorage.getItem("octopus:pending-new-session")).not.toBeNull();
  ...
  it("discards hand-offs older than the max age", () => {
    ...
    window.sessionStorage.setItem("octopus:pending-new-session", stale);
    expect(consumePendingNewSession()).toBeNull();
    // And the stale entry is removed.
    expect(window.sessionStorage.getItem("octopus:pending-new-session")).toBeNull();
  });
