...
describe("browser relay cursor overlay", () => {
  ...
  afterEach(() => {
    (
      globalThis as typeof globalThis & {
        __OCTOPUS_CURSOR_OVERLAY_STORE__?: { destroy?: () => void };
      }
    ).__OCTOPUS_CURSOR_OVERLAY_STORE__?.destroy?.();
    delete (
      globalThis as typeof globalThis & {
        __OCTOPUS_CURSOR_OVERLAY_STORE__?: unknown;
      }
    ).__OCTOPUS_CURSOR_OVERLAY_STORE__;
    document.getElementById("octopus-agent-cursor-overlay-host")?.remove();
    vi.unstubAllGlobals();
  });
  ...
  it("renders inside an isolated, click-through overlay at the target", async () => {
    ...
    const host = document.getElementById("octopus-agent-cursor-overlay-host");
