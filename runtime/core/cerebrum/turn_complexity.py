...
Operators configure each tier via:
  * env: ``OCTOPUS_MODEL_LOCAL`` / ``OCTOPUS_MODEL_VALUE`` /
    ``OCTOPUS_MODEL_PERFORMANCE``
  * service_provider keys: ``smart_routing.local`` /
    ``smart_routing.value`` / ``smart_routing.performance``
  * full kill-switch: ``OCTOPUS_SMART_ROUTING=off``
...
def is_smart_routing_enabled() -> bool:
    env_val = os.environ.get("OCTOPUS_SMART_ROUTING")
...
def _resolve_tier_model(tier: str) -> str | None:
    ...
    Resolution order per tier:
      1. Tier-specific env: ``OCTOPUS_MODEL_<TIER>``
    ...
    """
    upper = tier.upper()
    env_key = f"OCTOPUS_MODEL_{upper}"
    ...
    # by setting OCTOPUS_MODEL_LOCAL (e.g. "ollama/qwen2.5:7b") or
    # OCTOPUS_MODEL_PERFORMANCE. None → router escalates.
    return None
...
def estimate_turn_complexity(
    ...
    *,
    ...
    # Short single-line messages → value. (A heuristic OctopusRouter
...
def select_model_for_complexity(
    ...
    *,
    ...
    if user_model and user_model.strip() and user_model not in {"octopus-agent", "auto"}:
