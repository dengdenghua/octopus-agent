...
def _hard_workspace_sandbox_available() -> bool:
    """Whether native process execution has a kernel-enforced sandbox."""
    try:
...
def _workspace_sandbox_handles_ordinary_action(
    ...
    """Let the workspace sandbox handle routine local commands and edits.

    Codex's Ask-for-approval and Approve-for-me modes run ordinary workspace
    work directly and invoke the selected reviewer only when the sandbox
    boundary is crossed. Echo's older capability gate asked before every
    shell command or file edit, which caused duplicate approval prompts. This
    helper removes that preflight only when the server has selected the
    workspace sandbox and the risk is fully enforceable by that sandbox.
    """
