...
_log = logging.getLogger("octopus.ai_mode")
...
def _state_path() -> Path:
    ...
    Override via ``OCTOPUS_AI_MODE_PATH`` for tests / non-default
    install layouts. Otherwise lives under the data dir (next to
    feature flags / cron).
    """
    explicit = os.environ.get("OCTOPUS_AI_MODE_PATH")
    if explicit:
...
def current_ai_mode() -> AIMode:
    ...
    Resolution order:
      1. ``OCTOPUS_AI_MODE`` env var (operator override)
    ...
    """
    env_val = os.environ.get("OCTOPUS_AI_MODE")
...
def _detect_local_model() -> tuple[bool, str | None]:
    ...
    Looks for:
      * ``OCTOPUS_MODEL_LOCAL`` env (explicit operator config)
      * ``ollama`` on PATH + a local server on :11434
      * LM Studio default port 1234
    """
    if os.environ.get("OCTOPUS_MODEL_LOCAL"):
        return True, "OCTOPUS_MODEL_LOCAL configured"
