    ...
    def run(
        ...
        *,
        ...
        with trace_stage(
            ...
            span.set_attribute("octopus.graph.node_count", len(graph.nodes))
            ...
            try:
                ...
                if outcome.get("ok"):
                    span.set_attribute("octopus.gepa.canary_key", outcome.get("canary_key", ""))
                    span.set_attribute("octopus.gepa.canary_phase", outcome.get("phase", ""))
            except Exception as exc:  # noqa: BLE001
                _logger.debug("winner canary outcome record skipped: %s", exc)

            span.set_attribute("octopus.graph.completed", overall_ok)
            span.set_attribute("octopus.graph.steps_run", len(steps))
            return traj
