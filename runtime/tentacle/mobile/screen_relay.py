...
class ScreenRelay:
    ...
    async def add_subscriber(
        self, tentacle_id: str, ws_client: Any
    ) -> None:
        """添加订阅者.
        ...
        logger.info(
            ...
            _ws_remote_addr(ws_client), tentacle_id,
    ...
    async def remove_subscriber(self, ws_client: Any) -> None:
        ...
        if subscribed_ids:
            logger.info(
                ...
                _ws_remote_addr(ws_client), subscribed_ids,
            )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.debug(
                    ...
                    _ws_remote_addr(subscribers[i]), result,
                )

    async def _send_frame(
        self, ws: Any, data: bytes
    ) -> None:
        """发送帧数据到单个订阅者.
        ...
        return {
            ...
            "device_streams": {
                tid: {
                    ...
                    "has_screenshot": self._stream_states.get(tid, DeviceStreamState(tid)).last_jpeg is not None,
                }
