...
def _default_base_dir() -> Path:
    ...
    Uses ``OCTOPUS_DATA_DIR`` when set (tests, custom installs),
...
def upsert_many(
    ...
    *,
    ...
    Dedupe rule: case-insensitive title match against any EXISTING
    suggestion (regardless of status). On a hit we refresh
    ...
    with lock:
        ...
        by_title: dict[str, Suggestion] = {}
        for entry in raw["suggestions"]:
            s = Suggestion.from_dict(entry)
            if s is None:
                continue
            existing.append(s)
            by_title[s.title.lower().strip()] = s
...
def generate_suggestions(
    ...
    *,
    ...
    existing = read_bucket(project_root, base_dir=base)
    dismissed = [
        str(s.get("title") or "")
    ...
    merged = read_bucket(project_root, base_dir=base)
    return {
