...
Self-gating: no images, no CLIP tower, or ``OCTOPUS_IMAGE_SEMANTIC=0`` →
...
def _disabled() -> bool:
    return os.environ.get("OCTOPUS_IMAGE_SEMANTIC", "auto").strip().lower() in (
        "0",
        "false",
        "no",
        "off",
...
def ort_providers() -> list[str]:
    ...
    Read from ``OCTOPUS_ORT_PROVIDERS`` (comma-separated provider names, e.g.
    ...
    """
    raw = os.environ.get("OCTOPUS_ORT_PROVIDERS", "").strip()
    if not raw:
...
def embed_quantization() -> str | None:
    ...
    Read from ``OCTOPUS_EMBED_QUANTIZE`` (``int8`` / ``uint8`` / ``float32`` /
    ...
    """
    raw = os.environ.get("OCTOPUS_EMBED_QUANTIZE", "").strip().lower()
