...
Self-gating: no ``av``, no CLIP tower, or ``OCTOPUS_VIDEO_SEMANTIC=0`` →
...
def _disabled() -> bool:
    return os.environ.get("OCTOPUS_VIDEO_SEMANTIC", "auto").strip().lower() in (
        "0",
        "false",
        "no",
        "off",
...
def _transcribe_videos(videos: list[Path], root: Path, conn) -> int:
    ...
    INT8/INT8_FLOAT16 quantization can be enabled on capable hardware:
      * ``OCTOPUS_WHISPER_DEVICE``  (``cpu`` | ``cuda`` | ``auto``; default ``cpu``)
      * ``OCTOPUS_WHISPER_COMPUTE`` (``int8`` | ``int8_float16`` | ``float16``; default ``int8``)
      * ``OCTOPUS_WHISPER_MODEL``   (default ``small``)
    """
    try:
        from faster_whisper import WhisperModel
    except ImportError:
        return 0
    device = os.environ.get("OCTOPUS_WHISPER_DEVICE", "cpu").strip().lower() or "cpu"
    compute_type = os.environ.get("OCTOPUS_WHISPER_COMPUTE", "int8").strip() or "int8"
    model_name = os.environ.get("OCTOPUS_WHISPER_MODEL", "small").strip() or "small"
    try:
