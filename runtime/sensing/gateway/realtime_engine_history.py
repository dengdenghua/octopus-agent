...
_DELIVERED_KEY = "_host_history_delivered"
_NOTICE = (
...
class EngineHistory:
    ...
    def prompt(self, text: str, *, resumed: bool) -> str:
        delivered = self.session.metadata.get(_DELIVERED_KEY, set()) if self.session else set()
        prefix = self.missing_prefix if resumed else self.full_prefix
        if resumed and self.engine in delivered:
            prefix = ""
        return prefix + text

    def mark_delivered(self) -> None:
        # Only host-owned Session state; it is never projected from client metadata.
        if self.record_delivery is not None:
            self.record_delivery()
        if self.session is not None:
            self.session.metadata.setdefault(_DELIVERED_KEY, set()).add(self.engine)
...
def engine_history_for_turn(log: Any, turn: Any, engine: str) -> EngineHistory:
    ...
    return EngineHistory(_prefix(previous), _prefix(missing), engine, session, record_delivery)
