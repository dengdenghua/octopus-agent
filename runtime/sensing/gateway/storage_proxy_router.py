"""Same-origin gateway for the private octopus-storage service.

The browser talks only to ``/api/storage/*`` on octopus-agent.  The gateway
    ...
    async def proxy_storage(request: Request, storage_path: str) -> Any:
        ...
        try:
            ...
            if owns_client:
                await client.aclose()
            return JSONResponse(
                {"detail": "octopus-storage unavailable"},
                status_code=503,
