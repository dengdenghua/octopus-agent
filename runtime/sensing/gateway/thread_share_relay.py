...
class ThreadShareRelayClient:
    ...
    def from_env(cls) -> ThreadShareRelayClient | None:
        origin = str(os.environ.get("OCTOPUS_PUBLIC_SHARE_RELAY_URL") or "").strip()
        if not origin:
            return None
        return cls(
            origin,
            api_key=os.environ.get("OCTOPUS_PUBLIC_SHARE_RELAY_API_KEY"),
            bearer_token=os.environ.get("OCTOPUS_PUBLIC_SHARE_RELAY_BEARER_TOKEN"),
            timeout_seconds=float(
                os.environ.get("OCTOPUS_PUBLIC_SHARE_RELAY_TIMEOUT_SECONDS") or 10
            ),
        )
    ...
    def _request(
        ...
        *,
        ...
        if self.api_key:
            ...
            if owner_scope:
                headers["X-Octopus-Share-Owner-Scope"] = owner_scope
        else:
