...
flow instead of tribal knowledge. Five checks:
  ...
  4. 文档库 (octopus-storage)   — the File Agent data service is up
...
def ollama_url() -> str:
    return (os.environ.get("OCTOPUS_OLLAMA_URL") or "").strip().rstrip("/") or _DEFAULT_OLLAMA
...
def local_brain_status(
    *,
    ...
    items.append(
        {
            ...
            "action": (
                ...
                if (ollama_up and embed_models and embed_remote)
                    ...
                    "OCTOPUS_EMBED_URL=http://127.0.0.1:11434/v1 与 OCTOPUS_EMBED_MODEL=bge-m3。"
                )
            ),
        }
    )
    ...
    if ready:
        summary = "全部就绪:你的 octopus 完全跑在本地,一分钱云费都不花。"
    elif core_ok:
