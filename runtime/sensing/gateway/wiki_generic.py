...
``scripts/gen_wiki.py`` is hard-wired to Octopus's repo layout
...
Output layout (under ``<root>/.octopus-wiki/``)::
...
#
# Why filter ``_SKIP_DIRS`` + ``.octopus-wiki/``: the regen WRITES
# to ``.octopus-wiki/``, which fires more events, which would loop
...
_LOG = _logging.getLogger("octopus.wiki.watcher")
