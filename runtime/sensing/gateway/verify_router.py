...
def _exercise_page_with_visible_cursor(page: Any) -> None:
    page.evaluate(
        """() => {
            const existing = document.getElementById("__octopus_regression_cursor");
            if (existing) existing.remove();
            const cursor = document.createElement("div");
            cursor.id = "__octopus_regression_cursor";
    ...
    for x, y in [(120, 120), (360, 180), (640, 260), (900, 360), (1100, 460)]:
        ...
        page.evaluate(
            """([x, y]) => {
                const cursor = document.getElementById("__octopus_regression_cursor");
                if (cursor) {
