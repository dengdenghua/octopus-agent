...
``probe_provider(router, *, model, timeout_s)``
    ...
    ``~/.octopus/provider_caps.json``) so subsequent calls are free.
...
Disk cache
----------
Results are persisted to ``~/.octopus/provider_caps.json`` as a
...
_LOG = logging.getLogger("octopus.eyes.capability_probe")
...
def _disk_cache_path() -> Path:
    return Path.home() / ".octopus" / "provider_caps.json"
