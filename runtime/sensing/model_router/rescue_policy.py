"""Shared model-rescue policy for streamed agent loops.

The dispatch router, native tool loop, and text ReAct loop all need the same
answer to two questions: whether an upstream failure is retryable on another
provider, and which configured model should take over.  Keeping the policy in
one dependency-light module prevents the three execution paths from drifting.
"""
