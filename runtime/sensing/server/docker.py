...
class DockerBackend(LocalBackend):
    ...
    def sandbox(self, arm_id: str) -> Iterator[DockerSandbox]:
        with trace_stage("backend.docker.sandbox", arm_id=arm_id) as span:
            span.set_attribute("octopus.backend.image", self.image)
            span.set_attribute("octopus.backend.network_mode", self.network_mode)
            audit = BackendAudit(
                arm_id=arm_id,
...
class DockerSandbox(Sandbox):
    ...
    def run_command(
        ...
        *,
        ...
        container_name = f"octopus-{uuid.uuid4().hex[:12]}"
