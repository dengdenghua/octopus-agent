...
class K8sBackend(LocalBackend):
    ...
    def sandbox(self, arm_id: str) -> Iterator[K8sSandbox]:
        with trace_stage("backend.k8s.sandbox", arm_id=arm_id) as span:
            span.set_attribute("octopus.backend.k8s.image", self.image)
            span.set_attribute("octopus.backend.k8s.namespace", self.namespace)
            audit = BackendAudit(
                arm_id=arm_id,
            ...
            try:
                ...
                span.set_attribute("octopus.backend.k8s.runs", box.run_command_count)
                span.set_attribute("octopus.backend.timed_out_count", box.timeouts)
    ...
    def run_command(
        ...
        *,
        ...
        with trace_stage("backend.k8s.run_command", pod=pod_name) as span:
            span.set_attribute("octopus.backend.argv_len", len(argv))
            ...
            if result.get("timed_out"):
                self.timeouts += 1
                span.set_attribute("octopus.backend.timed_out", True)
                return {
