...
optional dependency that affects octopus's safety / evolution stack,
...
_LOG = logging.getLogger("octopus.preflight")
...
def _classify_features(
    ...
    # P3 — auto-checkpoint
    raw = (env.get("OCTOPUS_CHECKPOINT_EVERY_N") or "").strip()
    try:
        n = int(raw)
    except ValueError:
        n = 0
    ...
    raw_ets = (env.get("OCTOPUS_ENABLE_TRUST_SIGNAL") or "").strip().lower()
    ...
    # P1 — guard telemetry
    if (env.get("OCTOPUS_DISABLE_GUARD_TELEMETRY") or "").strip() == "1":
...
def _build_warnings(
    ...
    warnings: list[str] = []
    if (env.get("OCTOPUS_CHECKPOINT_MIRROR_URL") or "").strip() and not deps.get("redis", False):
        warnings.append(
            "OCTOPUS_CHECKPOINT_MIRROR_URL is set but `redis` package "
    ...
    n_raw = (env.get("OCTOPUS_CHECKPOINT_EVERY_N") or "").strip()
    if n_raw:
        try:
            n = int(n_raw)
            if n < 0:
                warnings.append(
                    f"OCTOPUS_CHECKPOINT_EVERY_N={n_raw!r} parses negative — "
                    "treated as off, did you mean a positive integer?",
                )
        except ValueError:
            warnings.append(
                f"OCTOPUS_CHECKPOINT_EVERY_N={n_raw!r} is not an int — "
...
def render_text(result: PreflightResult) -> str:
    lines = ["Octopus preflight (P0/P1/P2/P3 dress rehearsal)", ""]
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "for octopus's P0/P1/P2/P3 stack."
        ),
    )
    parser.add_argument(
