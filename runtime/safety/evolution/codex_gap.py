...
CAPABILITIES: tuple[GapCapability, ...] = (
    ...
    GapCapability(
        ...
        next_actions=(
            "Keep Chrome relay, bookmarklet fallback, and desktop app policy readiness green before raising browser/desktop claims.",
    ...
    GapCapability(
        id="tool_threat_model",
        area="octopus_advantage",
        title="High-risk tool threat model",
        why="Octopus can expose local power safely by keeping every risky tool edge tied to explicit controls.",
        target_score=0.95,
        implementation_paths=(
        ...
        next_actions=(
            "Keep threat-model controls mandatory for every new local tool class.",
            "Keep plugin lifecycle audit tied to compatibility summaries.",
        ),
    ),
    GapCapability(
        id="multi_agent_orchestration_readiness",
        area="octopus_advantage",
        title="Multi-agent orchestration readiness",
        why="Octopus can beat one-shot subagents by combining streaming delegation, worktree isolation, fitness routing, topology promotion, and measured lift.",
        target_score=0.95,
        implementation_paths=(
    ...
    GapCapability(
        id="swarm_scale_readiness",
        area="octopus_advantage",
        title="Swarm-scale orchestration readiness",
        why="Kimi-style clusters are strongest at large task fan-out; Octopus needs bounded scheduling, DAG contracts, streaming timelines, speedup evidence, and failure isolation.",
        target_score=0.95,
        implementation_paths=(
        ...
        test_paths=(
            ...
            "tests/test_parallel_agents_router_security.py",
        ),
        next_actions=(
            "Attach live production batch metrics to swarm readiness after each large fan-out run.",
            "Expose critical-path speedup and failed-task isolation in the operator panel.",
        ),
    ),
    GapCapability(
        id="repo_context_readiness",
        area="octopus_advantage",
        title="Repository context readiness",
        why="Claude and Cursor are strongest at repository awareness; Octopus needs shared wiki+source grounding, trace-backed working-set resume, and learning-memory quality recall.",
        target_score=0.95,
        implementation_paths=(
        ...
        next_actions=(
            "Attach repo-context source citations to every code-mode trace.",
            "Keep working-set resume and memory quality summaries visible in operator drill-downs.",
        ),
    ),
    GapCapability(
        id="product_experience_readiness",
        area="octopus_advantage",
        title="Product-experience true gap readiness",
        why="Cursor is strongest at product polish; Octopus needs true best-competitor gaps to stay visible, queueable, typed, and regression-tested.",
        target_score=0.95,
        implementation_paths=(
        ...
        next_actions=(
            "Keep true best-competitor gaps visible even when Octopus clears the target score.",
            "Attach operator remediation queues to every remaining product-experience gap.",
        ),
    ),
    GapCapability(
        id="model_provider_runtime_readiness",
        area="octopus_advantage",
        title="Model provider runtime readiness",
        why="Codex has a first-party OpenAI runtime; Octopus can lead on explicit OpenAI-compatible and domestic-provider compatibility evidence.",
        target_score=0.95,
        implementation_paths=(
        ...
        next_actions=(
            "Keep provider payload-shape probes covering strict domestic OpenAI-compatible endpoints.",
            "Export provider compatibility failures into replay-gated repair samples without leaking secrets.",
        ),
    ),
    GapCapability(
        id="governance_audit",
