...
TASK_SPEC_SCHEMA = "octopus.evolution.task_spec.v1"
TRIAL_SCHEMA = "octopus.evolution.trial.v1"
PAIR_EVIDENCE_SCHEMA = "octopus.evolution.pair_evidence.v1"

_STORE_LOCK = threading.RLock()
_ENGINES = frozenset({"octopus", "codex"})
...
class ExperimentTrial:
    ...
    def __post_init__(self) -> None:
        ...
        if self.engine not in _ENGINES:
            raise ValueError("trial engine must be octopus or codex")
...
def build_pair_evidence(
    ...
    *,
    ...
    wins = {"octopus": 0, "codex": 0, "tie": 0}
    for pair_key, strand in groups.items():
        if set(strand) != _ENGINES:
            continue
        octopus = strand["octopus"]
        codex = strand["codex"]
        winner = _winner(octopus, codex, primary_metric=primary_metric)
        wins[winner] += 1
        pairs.append(
            {
                "pair_key": pair_key,
                "experiment_id": octopus.experiment_id,
                "case_id": octopus.task_spec.case_id,
                "task_spec_hash": octopus.task_spec.task_spec_hash,
                "trial_index": octopus.trial_index,
                "goal": octopus.task_spec.goal,
                "domain": octopus.task_spec.domain,
                "winner": winner,
                "octopus": octopus.to_wire(),
                "codex": codex.to_wire(),
            }
        )
...
def _winner(
    octopus: ExperimentTrial,
    codex: ExperimentTrial,
    *,
    primary_metric: str,
) -> str:
    # Hard gates and real outcome dominate every efficiency metric.
    octopus_valid = octopus.hard_gate_passed and bool(octopus.outcome_passed)
    codex_valid = codex.hard_gate_passed and bool(codex.outcome_passed)
    if octopus_valid != codex_valid:
        return "octopus" if octopus_valid else "codex"
    if not octopus_valid:
        return "tie"
    octopus_metric = octopus.metrics.get(primary_metric)
    codex_metric = codex.metrics.get(primary_metric)
    if octopus_metric is None or codex_metric is None:
        return "tie"
    if math.isclose(octopus_metric, codex_metric, rel_tol=1e-9, abs_tol=1e-9):
        return "tie"
    return "octopus" if octopus_metric > codex_metric else "codex"
