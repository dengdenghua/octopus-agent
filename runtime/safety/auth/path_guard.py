    ...
    *,
    ...
    if sandbox_dir is not None:
        try:
            ...
            # Oversized tool results live in a private session-scoped spill
            # directory outside the project. Permit only the exact artifact
            # capability created for this active session; every other external
            # path remains blocked by the workspace boundary.
            try:
                from runtime.execution.tool_engine.tool_output_spill import (
                    is_current_session_spill_path,
                )

                is_owned_spill = is_current_session_spill_path(resolved)
            except (ImportError, OSError, RuntimeError):
                is_owned_spill = False
            if not is_owned_spill:
                return PathVerdict(
                    False,
                    p_in,
