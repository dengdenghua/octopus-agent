...
def run_code_command(args: Any, *, color: bool = True) -> int:  # noqa: ARG001
    ...
    payload = {
        ...
        "continue_command": "octopus-agent code --continue",
        "resume_command": f"octopus-agent code --resume {session_id}",
    }
...
def _resolve_model(args: Any, session_record: dict[str, Any]) -> str:
    for value in (
        ...
        os.environ.get("OCTOPUS_MODEL"),
        os.environ.get("OCTOPUS_MODEL_VALUE"),
...
def _sessions_dir() -> Path:
    ...
    root = Path(home).expanduser() if home else Path.home() / ".octopus"
