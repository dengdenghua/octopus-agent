"""Activate a staged migration into octopus's live stores (the "B" step).
...
* **memory** → appends bounded, idempotent index entries into the project's
  ``.octopus/MEMORY.md`` (the project-scope memory octopus's recall reads). The
  full imported text stays under ``.octopus/imported/<source>/memory/``.
...
def _imported_root(project_root: Path) -> Path:
    return Path(project_root) / ".octopus" / "imported"
...
def activate_memory(project_root: Path, *, sources: set[str] | None = None) -> tuple[int, int]:
    ...
    if not root.is_dir():
        return (0, 0)
    index = Path(project_root) / ".octopus" / "MEMORY.md"
