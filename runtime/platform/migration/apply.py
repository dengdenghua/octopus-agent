"""Apply a (reviewed) migration plan into octopus — the write step.

Staging-first and safe by default:

* **skills** → copied into ``.octopus/imported/<source>/skills/<name>/`` and
...
* **memory / rules / agents / commands** → copied into
  ``.octopus/imported/<source>/<kind>/`` for review. NOT auto-merged into
  octopus's live memory — semantics differ per tool, so activation is explicit.
* **mcp servers** → recorded to ``.octopus/imported/<source>/mcp.disabled.json``.
...
def apply_plan(
    ...
    *,
    ...
    """Materialize ``plan`` under ``<project_root>/.octopus/imported/<source>/``.

    ``kinds`` (optional) restricts which item kinds to apply (e.g. ``{"skill"}``).
    """
    base = Path(project_root) / ".octopus" / "imported" / plan.source
