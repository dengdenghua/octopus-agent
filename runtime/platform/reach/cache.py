...
def _default_cache_path() -> Path:
    configured = os.environ.get("OCTOPUS_REACH_CACHE_PATH")
    if configured:
        return Path(configured).expanduser()
    root = Path(os.environ.get("OCTOPUS_HOME") or (Path.home() / ".octopus"))
    return root / "cache" / "reach.sqlite3"


reach_cache = ReachCache(
    max_entries=int(os.environ.get("OCTOPUS_REACH_CACHE_MAX_ENTRIES", "512")),
