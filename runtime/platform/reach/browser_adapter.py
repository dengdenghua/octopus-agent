...
def read_with_browser(url: str, *, timeout_ms: int, max_bytes: int) -> dict[str, Any]:
    """Read through Octopus' active Chrome/Electron/Playwright browser track."""
    ...
    if result.get("error"):
        return {
            **result,
            ...
            "url": url,
            "requires_login": True,
            "repair_hint": "Sign in in Chrome or the Octopus browser, then retry with use_browser=true.",
        }
    return {
        **result,
        "ok": True,
