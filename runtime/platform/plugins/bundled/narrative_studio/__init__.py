...
class NarrativeStudioPlugin(ModulePlugin):
    ...
    def _resolve_data_dir(config: dict[str, Any]) -> Path:
        ...
        Desktop builds always set ``OCTOPUS_DATA_DIR`` and therefore use their
        sandboxed application data directory.  Older source-checkout versions
        stored Narrative Studio under ``~/.octopus/data``; when no shared-root
        ...
        if os.environ.get("OCTOPUS_DATA_DIR") or os.environ.get("OCTOPUS_HOME"):
            return preferred

        legacy = Path.home() / ".octopus" / "data" / "narrative-studio"
    ...
    def register_routes(self) -> None:
        ...
        async def mcp_json_rpc(body: dict[str, Any], request: Request) -> Any:
            ...
            # Production authentication is owned by the Octopus host.  This
            ...
            if self._host_requires_auth and actor is None:
                raise HTTPException(401, "authenticated Octopus principal required")
    ...
    def _discover_echo_root(*, configured: str, plugin_dir: Path) -> str:
        ...
        for ancestor in (resolved_plugin, *resolved_plugin.parents):
            if ancestor.name == "octopus-agent":
