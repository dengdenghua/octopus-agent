...
- 原 codex github 插件依赖 OpenAI 托管的 connector(.app.json 中的
  ...
  PR 摘要 / issue 分诊 / PR 列表 / 创建 PR 等核心能力,包装为 Octopus 的
  ModulePlugin。
- MIT 许可证沿用 OpenAI 原作者署名;connector 实现由 Octopus 完全重写,
  不依赖任何外部托管服务。
"""
...
class GitHubPlugin(ModulePlugin):
    ...
    author = "Octopus (ported from OpenAI codex github plugin, MIT)"
