...
可插拔、**带页面**的 octopus 插件模块:内置 A 股股票池 + 盘中随机游走模拟行情,
...
```
http://<octopus地址>/api/plugins/paper-trading/page
```
...
| GET | `/live/push/subscribe?event=&codes=` | 订阅推送事件(如 `kLineRealTime`, `todayStock`),返回最新快照 |
...
- 断线自动重连(重取 token / 重新签名 / 重新订阅),推送数据 gzip 自动解压。
- `light=true`(默认)时 `kLineRealTime` 会压缩为紧凑字段:code/name/price/change_pct/
  open/high/low/prev_close/volume/amount/换手/bids/asks,省流量。

**Python 内嵌 API(量化策略最方便)** —— 直接在插件进程内订阅回调:

```python
plugin = get_plugin("paper_trading")
push = plugin._push_client()               # 懒启动常驻 WS
push.subscribe("kLineRealTime", ["605080.sh", "003032.sz"], lambda ev, data: ...)
quote = push.latest("kLineRealTime")       # 最新快照(已解码)
push.add_global_callback(lambda ev, data: ...)  # 全事件回调
```

**HTTP/SSE**:页面/外部程序可 `GET /live/push/subscribe` 订阅后连 `/live/push/stream`
收 SSE(`event:` 为事件名,`data:` 为 JSON)。盯盘页(`/watch`)已改用 SSE 实时刷新,
仅在推送断开时降级回 10s 轮询。
...
- 账户 / 持仓 / 成交记录持久化到 `~/.octopus/data/paper_trading/state.json`,重启不丢。
- 自选股分组持久化到 `~/.octopus/data/paper_trading/watchlists.json`(与账户独立,重置账户不清空)。
