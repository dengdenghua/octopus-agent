(function () {
  ...
  function isTrustedParentOrigin(origin) {
    ...
    if (origin === "null" && window.location.protocol === "octopus-app:") return true;
  ...
  function requestQuoteConfig(reason) {
    ...
    window.parent.postMessage(
      { type: "octopus:quote-config-request", reason: reason || "initial" },
  ...
  window.addEventListener("message", function (event) {
    ...
    if (!message || message.type !== "octopus:quote-config" || message.version !== 1) return;
