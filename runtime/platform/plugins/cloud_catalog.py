...
_REMOTE_BASE = os.environ.get(
    ...
    "https://github.com/dengdenghua/workbuddy-expert-market/releases/download/octopus-content",
)
...
class CloudCatalog:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        prefer_remote = (
            self._force_remote_once or os.environ.get("OCTOPUS_CLOUD_STORE_PREFER_REMOTE") == "1"
        )
    ...
    def items(self) -> list[dict[str, Any]]:
        ...
        if self._kind == "plugins":
            ...
            if allow_dev_defaults:
                # The source-checkout descriptors are the authoritative local
                # development contract.  Replace every catalog collision (and
                # collapse duplicate polluted rows) instead of merely appending
    ...
    def installed_plugins(self) -> list[str]:
        ...
        来源合并四处,保证线上商城对「本地已有」的项直接标已安装:
          1. 随应用发布且已激活的工厂工作台
          2. 云安装落点 ~/.octopus/plugins/<kind>/<id>(本功能装的)
          3. Codex 格式插件 ~/.octopus/plugins/codex/<plugin>/.codex-plugin/plugin.json
             (首次自动从旧 ~/.codex/plugins/cache 同步)
          4. 连接器安装状态 ~/.octopus/connectors/state.json(installed=true)
        """
    ...
    def _commit_marketplace_skill_generation(
        ...
        *,
        ...
        generation = {
            "schema": "octopus.marketplace_skill_generation.v1",
    ...
    def _validated_marketplace_skill_generation(
        ...
        *,
        ...
        if not isinstance(payload, dict) or payload.get("schema") != (
            "octopus.marketplace_skill_generation.v1"
        ):
        ...
        *,
        ...
        for skill_id, source in expected.items():
            ...
            if source_meta.is_file():
                ignored_path = Path(".octopus-no-signature")
            else:
    ...
    def _marketplace_state_snapshot(
        ...
        *,
        ...
        return {
            "schema": "octopus.marketplace_state_snapshot.v1",
            "present": present,
    ...
    def _restore_marketplace_state(
        ...
        *,
        ...
        if (
            ...
            or snapshot.get("schema") != "octopus.marketplace_state_snapshot.v1"
    ...
    def _install_plugin_unlocked(
        ...
        *,
        ...
        if safe in _dependency_chain:
            ...
            dependency_kind = "workbench" if plugin_kind == "workbench" else "marketplace"
            raise ValueError(f"cyclic {dependency_kind} dependency: {chain}")
        ...
        with tempfile.TemporaryDirectory(prefix="octopus-plugin-") as tmp:
            ...
            if dev_install:
                ...
                extracted = self._extract_member(
                    self._archive_path(), member_prefix, Path(tmp), safe
                )
    ...
    def _rollback_marketplace_package(
        ...
        *,
        ...
        if (
            not isinstance(record, dict)
            or record.get("schema") != "octopus.marketplace_package_transaction.v1"
            or record.get("plugin_id") != safe
    ...
    def _commit_workbench_package(
        ...
        *,
        ...
        try:
            ...
            raise
        record = {
            "schema": "octopus.workbench_package_transaction.v1",
            "transaction_id": transaction_id,
            "plugin_id": plugin_id,
            "operation": operation,
            "status": "committed",
        ...
        *,
        ...
        try:
            ...
            atomic_write_json(
                ...
                {
                    "schema": "octopus.marketplace_package_intent.v1",
                    "transaction_id": transaction_id,
                    "plugin_id": plugin_id,
                    "kind": package_kind,
                    "operation": operation,
        ...
        record = {
            "schema": "octopus.marketplace_package_transaction.v1",
            "transaction_id": transaction_id,
            "plugin_id": plugin_id,
            "kind": package_kind,
            "operation": operation,
    ...
    def _validate_workbench_compatibility(
        ...
        *,
        ...
        if "host.same_origin" in manifest.permissions and not (
            official is not None and official.get("source") == "octopus"
        ):
    ...
    def _latest_marketplace_transaction(
        ...
        *,
        ...
        for path in transactions.glob("*.json"):
            ...
            if (
                isinstance(payload, dict)
                and payload.get("schema") == "octopus.marketplace_package_transaction.v1"
                and payload.get("plugin_id") == plugin_id
        ...
        *,
        ...
        for container in sorted(staging_root.iterdir()) if staging_root.is_dir() else []:
            ...
            if (
                ...
                or intent.get("schema") != "octopus.marketplace_package_intent.v1"
