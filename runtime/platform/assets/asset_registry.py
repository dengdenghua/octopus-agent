...
来源聚合:
  * 插件:Codex 插件(~/.octopus/plugins/codex) + WorkBuddy 连接器(extensions/workbuddy-connectors)
  * 技能:本地技能(~/.octopus/skills) + 内置技能(runtime/execution/all_skills) + 迁移导入(.octopus/imported)
...
UNIFIED_ROOT = Path.home() / ".octopus" / "assets"
INDEX_FILE = UNIFIED_ROOT / "index.json"

# 各来源根
CODEX_PLUGIN_ROOT = Path.home() / ".octopus" / "plugins" / "codex"
CONNECTOR_ROOT = REPO / "extensions" / "workbuddy-connectors" / "connectors"
CONNECTOR_MANIFEST = REPO / "extensions" / "workbuddy-connectors" / "octopus-manifest.json"
LOCAL_SKILLS = Path.home() / ".octopus" / "skills"
