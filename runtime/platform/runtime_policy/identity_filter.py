...
# -----------------------
#
# In a multi-agent team the default brand "Octopus" is wrong for
# sub-agents · e.g. when the Coder agent's underlying Claude leaks
# "I'm Claude" we want to rewrite it to "I'm Coder" (the agent's
# display_name), not "I'm Octopus". Hardcoding one brand for the whole
...
# ("built by the Octopus team"), not the *speaker*.
...
def _env_lock_enabled() -> bool:
    ...
    Env values that UNLOCK (all case-insensitive):
        OCTOPUS_IDENTITY_LOCK=0 | false | off | no
    """
    if _RUNTIME_OVERRIDE is not None:
        return _RUNTIME_OVERRIDE
    v = os.environ.get("OCTOPUS_IDENTITY_LOCK", "").strip().lower()
