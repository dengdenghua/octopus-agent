...
# ═══════════════════════════════════════════════════════════
# Workspace root resolution · lives under $OCTOPUS_DATA_DIR
# ═══════════════════════════════════════════════════════════
#
# We anchor workspaces under ``$OCTOPUS_DATA_DIR`` (falls back to CWD) so
...
def resolve_write_scope(session: Session | None) -> WriteScope:
    ...
    * Mode ``team`` requires ``session.metadata['team_id']`` — without
    ...
    if requested == "code" and _agent_has_code_mode(session):
        ...
        #     .octopus-work/<thread_id>/``. The AI writes only into
            ...
            try:
                ...
                if wp_path.is_absolute():
                    ...
                    if sb_mode == "sandbox":
                        ...
                        sandboxed = wp_path / ".octopus-work" / thread_id
