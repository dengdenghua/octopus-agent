...
_LOG = logging.getLogger("octopus.credentials.secret_store")
...
# and revoke by hand (`security find-generic-password -s octopus-agent`).
_SERVICE = "octopus-agent"
...
def keychain_backend() -> str | None:
    ...
    ``OCTOPUS_KEYCHAIN=off`` disables keychain access entirely, for
    containers and CI where the helpers exist but no keyring daemon does.
    """
    if (os.environ.get("OCTOPUS_KEYCHAIN") or "").strip().lower() in {
        "off",
        "0",
        "false",
        "disabled",
