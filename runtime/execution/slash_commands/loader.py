...
Community users can drop files in `~/.octopus/commands/` and
...
def _global_commands_dir() -> Path:
    """~/.octopus/commands/ · overrideable via $OCTOPUS_HOME."""
    home = os.environ.get("OCTOPUS_HOME")
    if home:
        return Path(home) / "commands"
    return Path.home() / ".octopus" / "commands"


def _project_commands_dir(project_dir: Path | str | None) -> Path | None:
    """Project-local <dir>/.octopus/commands/ · None if no project dir."""
