...
them into Octopus should first produce a normalized manifest that the UI can
...
def _infer_plugin_layout(plugin_dir: Path) -> list[str]:
    ...
    if (plugin_dir / ".mcp.json").is_file():
        present.append("mcp")
    if (
        (plugin_dir / "octopus-app.jsonc").is_file()
        or (plugin_dir / ".octopus-app.jsonc").is_file()
        ...
        for name, app_data in _iter_app_manifest_entries(data, manifest, warnings):
            ...
            modules.append(
                PackModule(
                    ...
                    path=str(
                        plugin_dir
                        if manifest.name in {"octopus-app.jsonc", ".octopus-app.jsonc", ".app.json"}
