...
const setEnabled=(next)=>{enabled=next;document.documentElement.classList.toggle('office-inspecting',enabled);if(!enabled)clear();parent.postMessage({type:'octopus:office:state',active:enabled},'*')};
...
const choose=(target)=>{if(!enabled)return;const node=target.closest('[data-office-node]');if(!node)return;clear();selected=node;node.classList.add('office-selected');parent.postMessage({type:'octopus:office:select',payload:{node:node.dataset.officeNode||'',label:node.dataset.officeLabel||'',text:readable(node)}},'*');setEnabled(false)};
addEventListener('message',(event)=>{const type=event.data&&event.data.type;if(type==='octopus:office:enable')setEnabled(true);if(type==='octopus:office:disable')setEnabled(false)});
addEventListener('click',(event)=>choose(event.target));addEventListener('keydown',(event)=>{if(enabled&&(event.key==='Enter'||event.key===' ')){choose(event.target);event.preventDefault()}});parent.postMessage({type:'octopus:office:ready'},'*')})();"""
