"""Shared Echo tools for external execution engines.

External engines start with an isolated tool/plugin/skill configuration.  This module is the narrow capability bridge back into the
*current Octopus turn*: it snapshots the already tenant-filtered, agent-
allowlisted tool catalog, advertises only those JSON schemas, and sends every
``item/tool/call`` request through the existing Octopus executor.
...
class HostToolRequest:
    """Adapter-owned coordinates; never resolve policy from engine arguments."""
...
def dynamic_tool_failure(reason: Any) -> dict[str, Any]:
    ...
    message = _bounded_text(reason) or "Octopus dynamic tool request was denied"
    return {
...
def validate_dynamic_tool_response(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or type(value.get("success")) is not bool:
        return dynamic_tool_failure("invalid Octopus dynamic tool response")
    raw_items = value.get("contentItems")
    if not isinstance(raw_items, list) or len(raw_items) > 16:
        return dynamic_tool_failure("invalid Octopus dynamic tool content")
...
def _advertised_name(skill_name: str, used: set[str]) -> str:
    ...
    candidate = f"octopus_{stem}_{digest}"[:_MAX_TOOL_NAME_CHARS]
    counter = 1
    while candidate in used:
        suffix = f"_{counter}"
        candidate = f"octopus_{stem}_{digest}"[: _MAX_TOOL_NAME_CHARS - len(suffix)] + suffix
        counter += 1
    used.add(candidate)
    return candidate
...
def _selected_plugin_ids(goal: str, context: Mapping[str, Any]) -> tuple[str, ...]:
    """Return bounded, explicit Octopus plugin selections for this turn.
...
def _catalog_agent(agent: Any, policy: Any) -> Any:
    ...
    return SimpleNamespace(
        agent_id=str(getattr(agent, "agent_id", "") or "engine-role"),
        arms=(
            SimpleNamespace(
                arm_id="engine-effective-turn-policy",
...
class HostToolBroker:
    """Execute one frozen host tool catalog for one exact engine turn."""
    ...
    def __init__(
        ...
        *,
        ...
        if not 1 <= max_tools <= _MAX_DYNAMIC_TOOLS:
            raise ValueError("host tool catalog limit must be between 1 and 128")
        executor = getattr(stack, "executor", None)
        registry = getattr(executor, "registry", None)
        if executor is None or registry is None:
            raise ValueError("Host tools require the Octopus execution stack")
        self._stack = stack
        self._agent = agent
        ...
        with session_scope(catalog_session):
            ...
            specs = build_anthropic_tool_specs(
                ...
                # Use the same role + explicit turn-grant union as Octopus'
        ...
        for spec in specs[:max_tools]:
            ...
            description = _bounded_text(
                getattr(spec, "description", "") or f"Run Octopus skill {skill_name}.",
    ...
    async def invoke(self, tool: str, arguments: dict[str, Any], *, call_id: str) -> dict[str, Any]:
        """Invoke a tool using coordinates already bound by the host adapter."""
        return await self(
            HostToolRequest(
    ...
    async def _handle(self, request: HostToolRequest) -> dict[str, Any]:
        ...
        if self._interrupted():
            return dynamic_tool_failure("outer Octopus turn was interrupted")
        ...
        if namespace not in (None, "", "octopus"):
        ...
        if entry is None:
            return dynamic_tool_failure("dynamic tool is not in this turn's Octopus catalog")
        ...
        if action == "deny":
            return dynamic_tool_failure(
                f"denied by Octopus approval policy (risk={risk.level}: {risk.reason})"
            )
        ...
        if action in {"ask", "confirm"} and not auto_approve:
            outer_request = OctopusApprovalRequest(
                ...
                detail=(
                    "The execution engine requested an Echo tool "
            ...
            try:
                ...
                return dynamic_tool_failure("Octopus approval failed closed")
        ...
        try:
            ...
            _logger.warning("Octopus dynamic tool failed (%s)", error_type)
            result = dynamic_tool_failure(f"Octopus dynamic tool failed ({error_type})")
        else:
