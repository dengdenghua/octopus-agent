...
-- octopus_effect_fenced_set_retain_v1
...
-- octopus_effect_fenced_set_release_v1
local current = redis.call('GET', KEYS[1])
if current ~= ARGV[1] then
  return 0
end
...
-- octopus_effect_fenced_delete_release_v1
local current = redis.call('GET', KEYS[1])
if current ~= ARGV[1] then
  return 0
end
...
-- octopus_effect_repair_committed_v1
...
class RedisEffectStore:
    ...
    def __init__(
        ...
        client: Any,
        *,
        key_prefix: str = "octopus:tool-effect:",
    ) -> None:
        if not key_prefix:
