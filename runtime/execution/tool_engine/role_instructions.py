"""Shared role, mode, memory and explicit Skill instructions for execution engines.

An external engine is not a second persona registry. The functions
here translate an ordinary ``Agent`` (soul, memory, modes and Octopus skills)
...
def _prompt_skill_roots() -> tuple[Path, ...]:
    ...
    if not immutable_prompt_catalog_required():
        roots.append(app_paths().data_dir / "skills")
    resolved: list[Path] = []
    for root in roots:
        try:
...
def resolve_explicit_skill_instructions(
    ...
    *,
    ...
    trusted Octopus prompt-skill roots participate. Unknown or action/plugin
    ...
    for name in names:
        ...
        if body:
            sections.append(
                f"<explicit-octopus-skill name={name!r}>\n{body}\n</explicit-octopus-skill>"
            )
    if not sections:
        return ""
    return (
        "The user explicitly selected the following Octopus prompt skill(s). "
        ...
        "the advertised Octopus dynamic tools.\n\n" + "\n\n".join(sections)
    )
...
def compose_role_instructions(
    ...
    *,
    ...
    """Render the same role/mode contract used by Octopus' native engine."""
    ...
    sections.append(
        ...
        "不要为检索自行启动另一套引擎或借用其他账号。引用返回的真实 URL，"
        "区分搜索失败与没有匹配结果。</search-policy>"
    )
    return _bounded(
