"""File Agent document search via the octopus-storage sibling service.

``octopus-storage`` is the Octopus family's local secure data cerebellum — the
...
same default the frontend uses, overridable via ``OCTOPUS_STORAGE_URL``.
"""
...
def _base_url() -> str:
    raw = (os.environ.get("OCTOPUS_STORAGE_URL") or "").strip()
...
def _unavailable() -> dict[str, Any]:
    return {
        ...
        "count": 0,
        "message": (
            f"本地文档库(octopus-storage)未运行或不可达({_base_url()})。"
...
def _search_documents(
    ...
    **kw: Any,
    ...
    """Search the user's local documents through octopus-storage and return
...
_SEARCH_DOCUMENTS_DESCRIPTION = (
    ...
    "and return source-cited snippets. Backed by the octopus-storage service "
    ...
    "octopus-agent never builds a document index itself.\n"
    "\n"
