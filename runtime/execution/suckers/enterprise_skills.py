"""Octopus Enterprise 企业服务 Arm 接入.

``octopus-enterprise`` 是 Octopus 家族的企业版后端 —— 独立 FastAPI
服务,管理项目 / 任务 / 审批 / 人员 / 文档。octopus-agent 通过本
...
def _base_url() -> str:
    raw = (os.environ.get("OCTOPUS_ENTERPRISE_URL") or "").strip()
    return (raw or _DEFAULT_URL).rstrip("/")


def _api_token() -> str | None:
    return (os.environ.get("OCTOPUS_ENTERPRISE_TOKEN") or "").strip() or None
...
def _unavailable() -> dict[str, Any]:
    return {
        ...
        "available": False,
        "message": (
            f"企业版服务(octopus-enterprise)未运行或不可达({_base_url()})。"
...
_ENTERPRISE_LIST_TASKS_DESC = (
    "List tasks in an Octopus Enterprise project. Backed by the "
    "octopus-enterprise backend (the enterprise management Arm).\n"
    "\n"
...
_ENTERPRISE_LIST_APPROVALS_DESC = (
    "List approval requests in Octopus Enterprise. Backed by the "
    "octopus-enterprise backend.\n"
    "\n"
...
_ENTERPRISE_LIST_PERSONS_DESC = (
    "List persons (team members / stakeholders) in Octopus Enterprise.\n"
    "\n"
