...
def register_delegation_skills(registry: SkillRegistry) -> int:
    ...
    description = (
        ...
        "default allowlist.\n"
        "For a file handoff, pass input_files and output_files as file path lists. "
        "Use isolate=true for a candidate code change in a host-created worktree. "
        "It includes current uncommitted project files and returns a durable patch "
        "for verification and application. "
        "The host snapshots inputs, verifies required output files, and returns "
        "artifacts with hashes and producer identity. A failed file handoff is "
        "not retried automatically; inspect its result before redispatching.\n"
        "\n"
    ...
    parallel_description = (
        ...
        "off (a worktree costs a checkout). The host snapshots current working files, "
        "including uncommitted changes, and returns a durable `worktree.patch` "
        "reference with a `diff` preview. Changes require verification and explicit "
        "application to the project. The worktree lives until the actual child stops; "
        "failed or cancelled attempts preserve partial patches and do not auto-retry. "
        "The host chooses the directory within the task's approved output scope.\n"
        "\n"
    ...
    tournament_description = (
        ...
        "and durable patch reference are returned for review and an explicit "
        "application decision. Each candidate starts from the approved project's "
        "working files, including uncommitted changes.\n"
        "\n"
        ...
        "path (must match the host-approved project), input_files?: list[str], "
        "output_files?: list[str], max_workers?: int (default 4), "
        "timeout_s?: int (per child; the parent deadline still applies)}.\n"
        "\n"
        "Returns: {ok, goal, decided_by (judge|only_candidate|judge_abstained|"
        "none), candidate_count, viable_count, winner:{id, files, diff, worktree}, "
        "candidates:[{id, ok, viable, files, error, worktree?}], selection_id, "
        "spawns_used, retry_allowed:false, note}. Failed candidates retain their "
        "patch or retained_workspace coordinates. Producers and read-only judges "
        "share the spawn budget, deadline and cancellation. Do not automatically "
        "repeat a tournament after a partial or interrupted run."
    )
    registry.register(
        Skill(
            name="tournament",
