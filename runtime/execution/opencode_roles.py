    ...
    with execution_request_scope(request), session_scope(session):
        async with (
            ...
            async with contextlib.AsyncExitStack() as lifetime:
                ...
                key = backend.model_key(scope, model)
                if backend.native_model(model)[0] == "echo-shared":
                    from runtime.execution.codex_backend.responses_proxy import (
                        CodexResponsesScope,
                        ScopedResponsesProxy,
                    )
                    from runtime.execution.model_services import (
                ...
                async with contextlib.aclosing(
                    backend.stream_prompt(
                        ...
                        reasoning_effort=effective_context.get("reasoning_effort"),
                        system=system,
                        model=model,
                    ...
                    async for event in events:
                        event = observe_engine_event(event, "opencode", model)
