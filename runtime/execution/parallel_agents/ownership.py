...
class OwnershipMixin:
    ...
    Visibility rule (consistent across all methods):
      - Batches with ``owner_id is None`` are visible to everyone
        (legacy state + single-user dev mode).
      - Batches with a specific owner_id are visible only to that owner.
      - Passing ``owner_id=None`` from the caller side returns ALL
        batches (used in dev mode where _auth returns None).
    """
