...
``<workspace>/.octopus/artifacts/<root_thread_id>/<sub_thread_id>/<sanitized-name>``

When no ``workspace_path`` is available it falls back to the project's
``.octopus/artifacts/`` root (mirroring how memory skills locate the repo
...
def _artifacts_dir(
    ...
    return base / ".octopus" / "artifacts" / root / sub
