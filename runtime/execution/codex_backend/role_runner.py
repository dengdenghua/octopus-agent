"""Unified Codex execution backend for ordinary Octopus roles.

The caller still resolves a normal ``Agent`` and its Octopus turn context.
...
class ServerCodexExecutionOverride:
    ...
    roles therefore ignore Octopus' ordinary ``model_name``/effort projection
...
def _timeout_s(context: Mapping[str, Any]) -> float:
    raw = context.get("timeout_s") or os.environ.get("OCTOPUS_CODEX_APP_SERVER_TIMEOUT")
    try:
...
def build_codex_role_request(
    ...
    *,
    ...
    if registry is None:
        raise CodexSecurityError("Codex role execution requires the Octopus tool registry")
    ...
    try:
        if request.use_system_model_proxy:
            ...
            if router is None or request.model is None:
                raise CodexSecurityError("Octopus Responses proxy is unavailable")
            try:
                ...
                raise CodexSecurityError("Octopus Responses proxy could not be started") from exc
            request = replace(
                request,
