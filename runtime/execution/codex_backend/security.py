"""Security boundary for an Octopus-managed Codex App Server sidecar.
...
resume/fork survives Octopus task attempts; each task gets fresh process and
...
PERMISSION_PROFILE = "octopus-sidecar"

_BINDING_KIND = "octopus-codex-thread-binding/v1"
...
class CodexThreadBinding:
    """Server-owned mapping from an Octopus thread to a Codex thread."""
...
def _cwd_hash(workspace: Path) -> str:
    return hashlib.sha256(f"octopus-codex-cwd\0{workspace}".encode()).hexdigest()
