"""Octopus-owned dynamic tools for the Codex App Server sidecar.
...
class CodexDynamicToolBroker:
    """Execute one frozen Octopus tool catalog for one exact Codex turn."""
    ...
    def __init__(
        ...
        *,
        ...
        if executor is None or registry is None:
            raise ValueError("Codex dynamic tools require the Octopus execution stack")
        self._stack = stack
        self._agent = agent
    ...
    async def _handle(self, request: AppServerRequest) -> dict[str, Any]:
        ...
        if action in {"ask", "confirm"} and not auto_approve:
            outer_request = OctopusApprovalRequest(
                ...
                detail=(
                    "Codex requested an Octopus dynamic tool "
