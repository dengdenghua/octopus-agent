...
executes an unreviewed upstream binary; promotion remains an Octopus build and
...
def resolve_bundled_codex_version() -> str:
    configured = os.environ.get("OCTOPUS_PACKAGED_CODEX_VERSION", "").strip()
    if configured:
        _version_parts(configured)
        return configured

    candidates = [app_paths().root / "frontend" / "package.json"]
    resources = os.environ.get("OCTOPUS_RESOURCES_DIR", "").strip()
    if resources:
