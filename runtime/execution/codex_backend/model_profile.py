...
The public UI chooses a policy (follow Octopus or use the Codex account) and
...
class CodexModelCompatibilityError(ConfigurationError):
    """The selected Octopus provider cannot safely run inside Codex."""


@dataclass(frozen=True, slots=True)
class CodexModelPreference:
    """Principal-scoped Coder preference; defaults to Octopus inheritance."""
...
def resolve_codex_execution_profile(
    *,
    ...
    if selected_preference.mode == "chatgpt":
        ...
        # Octopus system model could send a DeepSeek/Qwen/custom id to an
    ...
    # A missing Octopus default deliberately means "let Codex choose".  It
    ...
    return ResolvedCodexExecutionProfile(
        ...
        compatibility_reason=(
            ...
            if route_available
                ...
                "Octopus Responses proxy is unavailable for the selected system model"
                if not proxy_available
                else "Selected system model has no exact Octopus ModelRouter route"
            )
        ),
        proxy_required=True,
    )
