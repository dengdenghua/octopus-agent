...
def _platform_codex_state_root() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Octopus" / "codex_backend"
    if os.name == "nt":
        base = Path(os.environ.get("LOCALAPPDATA") or home / "AppData" / "Local")
        return base / "Octopus" / "codex_backend"
...
def resolve_codex_state_root(explicit: str | Path | None = None) -> Path:
    ...
    raw = explicit if explicit is not None else os.environ.get("OCTOPUS_CODEX_STATE_DIR")
    if raw is None or not str(raw).strip():
        ...
        # packaged app already supplies an external OCTOPUS_DATA_DIR; only the
