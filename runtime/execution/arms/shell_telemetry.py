...
octopus-agent's OpenTelemetry tracing infrastructure.
...
class ShellExecEvent:
    ...
    def to_span_attributes(self) -> dict[str, Any]:
        ...
        attrs: dict[str, Any] = {
            "octopus.shell.event_name": self.name,
            "octopus.shell.timestamp": self.timestamp,
        }
        for k, v in self.metrics.items():
            attrs[f"octopus.shell.metric.{k}"] = v
        for k, v in self.categories.items():
            attrs[f"octopus.shell.category.{k}"] = v
        for k, v in self.extra.items():
            attrs[f"octopus.shell.extra.{k}"] = v
        return attrs
