...
BASE_DIR=/opt/octopus-cloud/quote-hub
RELEASES_DIR=${BASE_DIR}/releases
CURRENT_LINK=${BASE_DIR}/current
ENV_FILE=/etc/octopus/quote-hub.env
SECRET_FILE=/etc/octopus/quote-hub-secret.json
SERVICE=octopus-quote-hub.service
SERVICE_USER=octopus-quote
SERVICE_GROUP=octopus-quote
...
install -d -o "${SERVICE_USER}" -g "${SERVICE_GROUP}" -m 0700 /var/lib/octopus/quote-hub
...
shopt -s nullglob
wheels=("${ARTIFACT_DIR}"/octopus_agent_runtime-*.whl)
shopt -u nullglob
