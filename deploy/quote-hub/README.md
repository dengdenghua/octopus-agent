...
```text
/opt/octopus-cloud/quote-hub/
├── current -> releases/20260825T120000Z-1234  # 原子软链
├── releases/<release>/.venv/                  # 每个版本独立 venv
└── deploy.lock

/etc/octopus/quote-hub.env                     # 0600
/etc/octopus/quote-hub-secret.json             # 0600
/var/lib/octopus/quote-hub/                     # token/cache，可写
```
...
sudo useradd --system --home /var/lib/octopus/quote-hub \
  --shell /usr/sbin/nologin octopus-quote
sudo install -d -o root -g root -m 0755 /etc/octopus
sudo install -d -o octopus-quote -g octopus-quote -m 0700 \
  /var/lib/octopus/quote-hub
sudo install -d -o root -g root -m 0755 \
  /opt/octopus-cloud/quote-hub/releases

sudo install -o octopus-quote -g octopus-quote -m 0600 \
  deploy/quote-hub/quote-hub.env.example /etc/octopus/quote-hub.env
sudo install -o octopus-quote -g octopus-quote -m 0600 \
  deploy/quote-hub/quote-hub-secret.json.example \
  /etc/octopus/quote-hub-secret.json
sudoedit /etc/octopus/quote-hub.env
sudoedit /etc/octopus/quote-hub-secret.json
```

秘密 JSON 的字段是 `phone` 与 `password`；不要把真实值写进 Git、shell 历史或
systemd unit。两个文件都必须是 `octopus-quote:octopus-quote`、权限 `0600`，否则
部署脚本会拒绝继续。
...
sudo install -o root -g root -m 0644 deploy/quote-hub/quote-hub.service \
  /etc/systemd/system/octopus-quote-hub.service
sudo systemd-analyze verify /etc/systemd/system/octopus-quote-hub.service
sudo systemctl daemon-reload
sudo systemctl enable octopus-quote-hub.service
```
...
scp -r "$artifact" root@47.85.24.213:/opt/octopus-cloud/quote-hub/incoming/
```
...
sudo deploy/quote-hub/deploy-release.sh \
  /opt/octopus-cloud/quote-hub/incoming/<artifact-directory>
```

核验：

```bash
sudo systemctl status octopus-quote-hub.service --no-pager
curl --fail http://127.0.0.1:8091/health
curl --fail http://127.0.0.1:8091/readyz
sudo journalctl -u octopus-quote-hub.service -n 100 --no-pager
```
...
```bash
ls -1 /opt/octopus-cloud/quote-hub/releases
