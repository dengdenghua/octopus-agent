...
把第一行结果写入 `OCTOPUS_LOCAL_AUTH_JWT_SECRET`，第二行 bcrypt 值写入
`OCTOPUS_ADMIN_PASSWORD_HASH`。生产环境推荐改用 ExternalSecrets/AWS Secrets
...
全量失效时轮换 `OCTOPUS_LOCAL_AUTH_JWT_SECRET` 并滚动重启。多人/企业生产应接入
...
```bash
kubectl label namespace ingress-nginx octopus-agent.io/ingress-access=true --overwrite
```
...
|---|---|---|---|---|
| `octopus-agent` | 1 | 100m / 500m | 256Mi / 1Gi | 基线 PVC 为 `ReadWriteOnce`，不可直接水平扩容 |
...
- 发布流程不推送 `latest`；SemVer tag 也只是可读指针，不是不可变供应链标识。
  ...
  `ghcr.io/dengdenghua/octopus-agent@sha256:<真实 digest>`；也可取消
...
- 当前 NetworkPolicy 对 Agent 入站默认拒绝，只允许带
  `octopus-agent.io/ingress-access=true` 的 controller namespace；Redis 只接受 Agent Pod。
