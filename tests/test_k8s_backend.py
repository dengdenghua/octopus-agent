...
class TestRequireAvailable:
    def test_missing_kubectl(self):
        m = K8sBackend()
        with patch(
            "runtime.sensing.server.k8s.shutil.which", return_value=None,
    ...
    def test_kubectl_version_fails(self):
        ...
        with patch(
            "runtime.sensing.server.k8s.shutil.which", return_value="/usr/bin/kubectl"
        ), patch(
            "runtime.sensing.server.k8s.subprocess.run", return_value=fake,
        ), pytest.raises(K8sUnavailableError):
            m.require_available()
...
class TestBuildKubectlArgv:
    ...
    def test_labels_added(self):
        box = self._box(labels={"app": "octopus"})
        argv = box._build_kubectl_argv("p", None, ["echo"])
        pairs = [argv[i + 1] for i, a in enumerate(argv) if a == "--labels"]
        assert "app=octopus" in pairs
...
class TestPodOverrides:
    ...
    def test_service_account(self):
        box = self._box(service_account="octopus-sa")
        ovr = box._build_pod_overrides()
        assert json.loads(ovr)["spec"]["serviceAccountName"] == "octopus-sa"
...
class TestRunCommand:
    ...
    def test_successful_run(self):
        ...
        assert r["pod"].startswith("octopus-")
    ...
    def test_timeout_triggers_cleanup(self):
        ...
        with patch(
            ...
            "runtime.sensing.server._streaming.stream_run", side_effect=fake_stream,
        ), patch(
            "runtime.sensing.server.k8s.subprocess.run", side_effect=fake_subprocess_run,
        ), m.sandbox("test") as box:
    ...
    def test_missing_kubectl_raises(self):
        ...
        with patch(
            "runtime.sensing.server.k8s.shutil.which", return_value=None,
        ), m.sandbox("test") as box, pytest.raises(K8sUnavailableError):
