...
def test_extension_visible_copy_uses_echo_brand() -> None:
    ...
    assert "Octopus Page Agent" not in bookmarklet
    assert "已发送到 Octopus" not in bookmarklet
    assert "Octopus Browser Relay:" not in background
...
const build = globalThis.OctopusMcpOAuthDeepLink.buildCallbackURL;
...
"""
    ...
    assert parsed["sent"]["type"] == "octopus.mcpOAuthDeepLink"
...
def test_cursor_overlay_is_early_nonblocking_and_brackets_each_command() -> None:
    ...
    assert 'type !== "octopus.cursorOverlay"' in overlay
    assert 'type: "octopus.cursorOverlayReady"' in overlay
    assert "const activeCursorOverlayByTab = new Map" in background
    assert "restorePageCursorOverlay(tabId)" in background
    assert 'type === "octopus.cursorOverlayReady"' in background
