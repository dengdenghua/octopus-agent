...
class TestMetricsRouter:
    ...
    def test_metrics_endpoint_returns_prometheus_format(self, fresh_registry):
        ...
        body = resp.text
        assert "# HELP octopus_test_calls_total Test counter" in body
        assert "# TYPE octopus_test_calls_total counter" in body
        assert 'octopus_test_calls_total{op="read"} 5' in body
        assert 'octopus_test_calls_total{op="write"} 2' in body

    def test_api_metrics_alias(self, fresh_registry):
        from fastapi.testclient import TestClient

        fresh_registry.counter("octopus_alias", "x").inc()
        client = TestClient(self._make_app())
        resp = client.get("/api/metrics")
        assert resp.status_code == 200
        assert "octopus_alias" in resp.text
...
class TestMetricsEndToEnd:
    """Beak emits ``octopus_skill_calls_total`` (Round 22). The
    ...
    def test_metrics_endpoint_shows_beak_emissions(self, fresh_registry):
        ...
        body = client.get("/metrics").text
        assert "octopus_skill_calls_total" in body
        assert 'sucker_id="ping"' in body
        # Histogram for latency too.
        assert "octopus_skill_latency_seconds" in body
