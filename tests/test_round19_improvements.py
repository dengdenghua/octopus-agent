"""Tests for Round-19 improvements:
...
class TestMetricsRegistry:
    ...
    def test_prometheus_render_counter(self):
        ...
        r = MetricsRegistry()
        c = r.counter("octopus_calls_total", "Total calls", labels=["skill"])
        c.inc(labels={"skill": "read_file"})
        out = r.render_prometheus()
        assert "# HELP octopus_calls_total Total calls" in out
        assert "# TYPE octopus_calls_total counter" in out
        assert 'octopus_calls_total{skill="read_file"} 1' in out
