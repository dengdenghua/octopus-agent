...
def test_vision_model_config_env(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("OCTOPUS_COMPUTER_VISION_MODEL", raising=False)
    monkeypatch.delenv("OCTOPUS_COMPUTER_VISION_BASE_URL", raising=False)
    monkeypatch.delenv("OCTOPUS_COMPUTER_VISION_API_KEY", raising=False)
    monkeypatch.delenv("OCTOPUS_COMPUTER_VISION_UPSTREAM_MODEL", raising=False)
    monkeypatch.setattr(cv, "Path", lambda *a, **kw: tmp_path / "missing.json")
    assert cv._vision_model_config("") is None

    monkeypatch.setenv("OCTOPUS_COMPUTER_VISION_MODEL", "m1")
    monkeypatch.setenv("OCTOPUS_COMPUTER_VISION_BASE_URL", "https://x")
    monkeypatch.setenv("OCTOPUS_COMPUTER_VISION_API_KEY", "key")
