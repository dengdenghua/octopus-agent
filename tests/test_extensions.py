...
def test_app_extension_called_with_app_and_context(monkeypatch):
    ...
    monkeypatch.setenv("OCTOPUS_APP_EXTENSIONS", "octo_test_appext")
...
def test_skill_extension_custom_func_and_count(monkeypatch):
    ...
    monkeypatch.setenv("OCTOPUS_SKILL_EXTENSIONS", "octo_test_skillext:my_register")
    reg: list = []
...
def test_multiple_comma_separated(monkeypatch):
    ...
    monkeypatch.setenv("OCTOPUS_SKILL_EXTENSIONS", "octo_ext_a, octo_ext_b")
    assert load_skill_extensions([]) == 3


def test_unconfigured_is_noop(monkeypatch):
    monkeypatch.delenv("OCTOPUS_APP_EXTENSIONS", raising=False)
    monkeypatch.delenv("OCTOPUS_SKILL_EXTENSIONS", raising=False)
...
def test_failing_extension_is_isolated(monkeypatch):
    ...
    monkeypatch.setenv("OCTOPUS_SKILL_EXTENSIONS", "octo_bad_ext")
    # 扩展抛错只记日志、不传播,返回 0。
    assert load_skill_extensions([]) == 0


def test_missing_module_isolated(monkeypatch):
    monkeypatch.setenv("OCTOPUS_APP_EXTENSIONS", "nonexistent_module_xyz")
    assert load_app_extensions("APP", AppExtensionContext()) == 0
