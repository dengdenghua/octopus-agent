...
class TestCrossoverLineage:
    def test_crossover_has_two_parents(self):
        ...
        router = _SeqRouter([
            "<reason>combine care + speed</reason>"
            "<suffix>be careful but quick</suffix>"
        ])
        mutator = PromptMutator(router=router, model="mock/m")
...
class TestLineageQuery:
    ...
    def test_deep_linear_lineage(self, stack):
        ...
        gen1 = PromptVariant(
            name="gen1", parents=("gen0",), generation=1, origin="mutation",
        )
        gen2 = PromptVariant(
            name="gen2", parents=("gen1",), generation=2, origin="mutation",
        )
    ...
    def test_retired_variant_still_queryable(self, stack):
        ...
        child = PromptVariant(
            name="c", parents=("a",), generation=1, origin="mutation",
        )
...
class TestAncestorsTree:
    ...
    def test_crossover_branches(self, stack):
        ...
        merged = PromptVariant(
            name="merged", parents=("a", "b"), generation=1, origin="crossover",
        )
    ...
    def test_deep_mixed_tree(self, stack):
        ...
        a = PromptVariant(name="a", generation=0)
        a1 = PromptVariant(
            name="a1", parents=("a",), generation=1, origin="mutation",
        )
        b = PromptVariant(name="b", generation=0)
        b1 = PromptVariant(
            name="b1", parents=("b",), generation=1, origin="mutation",
        )
        merged = PromptVariant(
            name="merged", parents=("a1", "b1"),
            generation=2, origin="crossover",
        )
        opt = PromptOptimizer(stack, [a, b])
        opt.add_variant(a1)
        opt.add_variant(b1)
