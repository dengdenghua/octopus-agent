...
def host(tmp_path, *, screenshot=False):
    ...
    request = ExecutionRequest(
        ExecutionTask(
            ...
            resources=ExecutionResources(5000, 1.0, time.monotonic() + 60),
        ),
    ...
    def probe():
        ...
        assert current_execution_request() is request
        return {
            "actor": active.actor,
...
async def test_read_only_task_can_read_but_cannot_write(tmp_path):
    host_bridge, _, workspace = host(tmp_path)
    request = replace(
        host_bridge.request,
        task=replace(
            host_bridge.request.task,
            permissions=replace(host_bridge.request.task.permissions, writable_roots=()),
        ),
    )
    bridge = HostMCPBridge(host_bridge.broker, request, host_bridge.session)
    async with (
        bridge.serve() as connection,
        httpx.AsyncClient(
            base_url=connection.url,
