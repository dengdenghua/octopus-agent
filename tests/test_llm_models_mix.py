"""The octopus-mix virtual model is advertised on /api/llm-models so the
...
def test_llm_models_advertises_octopus_mix() -> None:
    r = _client().get("/api/llm-models")
    assert r.status_code == 200
    models = r.json()["models"]
    by_id = {m["id"]: m for m in models}
    assert "octopus-mix" in by_id
    entry = by_id["octopus-mix"]
    assert entry["provider"] == "octopus"
    assert entry["supports_tool_use"] is True
    # listed first — the octopus-native flagship leads the catalog
    assert models[0]["id"] == "octopus-mix"
