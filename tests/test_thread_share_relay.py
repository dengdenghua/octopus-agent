...
def test_relay_create_list_and_revoke_use_narrow_account_routes(monkeypatch) -> None:
    ...
    owner_scope = first.get_header("X-octopus-share-owner-scope")
    assert owner_scope.startswith("relay_")
    assert len(owner_scope) == 70
    assert calls[1][0].get_header("X-octopus-share-owner-scope") == owner_scope
    assert calls[2][0].get_header("X-octopus-share-owner-scope") == owner_scope
...
def test_relay_bearer_uses_device_owner_scoped_routes(monkeypatch) -> None:
    ...
    assert request.get_header("X-octopus-share-owner-scope") is None
