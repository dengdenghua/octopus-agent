...
def test_reflex_greeting_uses_selected_agent_identity(tmp_path: Path) -> None:
    ...
    assert "我是 Octopus" not in assistant_items[-1]["text"]
...
def test_direct_text_classifier_does_not_require_native_model_router(tmp_path: Path) -> None:
    from runtime.protocol.items import TurnParams
    ...
    params = TurnParams.model_validate(
        {
            "threadId": "th-external-text-only",
            "input": [
                {
                    "type": "text",
                    "text": "只回复由 S P E E D Y 这些字母连成的单词",
                    "metadata": {"context": {"mode": "code", "capability_mode": "code"}},
                }
            ],
        }
    )

    assert _should_use_direct_text_path("只回复由 S P E E D Y 这些字母连成的单词", params)
