...
def test_usage_is_idempotent_and_survives_store_restart(
...
def test_actual_usage_trips_root_breaker_and_refuses_future_spawn(
    ...
    assert (
        store.acquire(
            "turn-1", depth=1, global_limit=64, root_limit=16, owner_id="worker-2"
        )
        is None
    )
    # The circuit is isolated to one root; a later human turn is unaffected.
    assert store.acquire(
        "turn-2", depth=1, global_limit=64, root_limit=16, owner_id="worker-2"
    )
...
def test_durable_leases_enforce_cross_worker_subtree_limit(tmp_path: Path) -> None:
    ...
    lease = worker_a.acquire(
        "turn-1", depth=1, global_limit=10, root_limit=1, owner_id="worker-a"
    )

    assert lease is not None
    assert (
        worker_b.acquire(
            "turn-1", depth=2, global_limit=10, root_limit=1, owner_id="worker-b"
        )
        is None
    )
    assert worker_a.release(str(lease["lease_id"])) is True
    assert worker_b.acquire(
        "turn-1", depth=2, global_limit=10, root_limit=1, owner_id="worker-b"
    )
...
def test_expired_process_lease_recovers_after_restart(tmp_path: Path) -> None:
    ...
    first = SubagentGovernanceStore(path)
    lease = first.acquire(
        "turn-1", depth=1, global_limit=1, root_limit=1, owner_id="dead-worker"
    )
    assert lease is not None
...
def test_cost_can_trip_breaker_when_token_volume_is_small(
    tmp_path: Path, monkeypatch
) -> None:
