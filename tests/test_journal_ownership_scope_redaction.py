...
def test_pii_scope_is_pseudonymized_without_collapsing_ownership(tmp_path) -> None:
    ...
    assert row["tenant_id"].startswith("octopus-scope-tenant-vone-")
    assert row["owner_actor_id"].startswith("octopus-scope-owner-vone-")
    assert row["tenant_id"] != row["owner_actor_id"]
    assert len(journal.read_all(scope=TenantScope(_TENANT, _OWNER))) == 1
    assert journal.read_all(scope=TenantScope("tenant-b", "bob@example.com")) == []


def test_reader_keeps_historical_raw_scope_rows_visible(tmp_path) -> None:
    path = tmp_path / "events.jsonl"
    JSONLJournal(path).write(_event())
    journal = JSONLJournal(path, redactor=Redactor())

    visible = journal.read_all(scope=TenantScope(_TENANT, _OWNER))
...
def test_streaming_subscribers_receive_exact_pseudonymized_scope(tmp_path) -> None:
    path = tmp_path / "events.jsonl"
    ...
    assert received[0].tenant_id.startswith("octopus-scope-tenant-vone-")
    assert received[0].owner_actor_id.startswith("octopus-scope-owner-vone-")
    assert received[0].model_dump(mode="json") == json.loads(path.read_text(encoding="utf-8"))


def test_payload_redaction_cannot_corrupt_structural_uuid(tmp_path) -> None:
    path = tmp_path / "events.jsonl"
    journal = JSONLJournal(path, redactor=Redactor())
...
def test_tool_effect_hashes_remain_exact_when_phone_shaped(tmp_path) -> None:
    path = tmp_path / "events.jsonl"
    journal = JSONLJournal(path, redactor=Redactor())
    ...
    assert row["effect_key"] == effect_key
    assert row["args_fingerprint"] == fingerprint


def test_packed_chunks_redact_payload_without_rewriting_structure(tmp_path) -> None:
    path = tmp_path / "events.jsonl"
    journal = JSONLJournal(path, redactor=Redactor())
    canary = "sk-ant-api03-abcdefghijklmnopqrstuvwxyz0123456789ABC"
    ids = [
    ...
    assert "__chunk_row__" in raw.splitlines()[0]
    assert canary not in raw
