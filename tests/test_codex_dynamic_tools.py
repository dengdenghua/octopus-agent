"""Security-boundary tests for the Octopus-owned Codex dynamic tool broker."""
...
async def test_broker_exception_response_and_log_do_not_expose_secret(
    ...
    assert rendered == "Octopus dynamic tool failed (RuntimeError)"
...
@pytest.mark.asyncio
async def test_explicit_octopus_plugin_grant_is_brokered_and_next_turn_revocation_is_exact(
    tmp_path: Path,
) -> None:
    calls: list[str] = []
    registry = SkillRegistry()
    ...
    result = await granted(
        _request(
            ...
            {"value": "ran-through-octopus"},
            call_id="plugin-call",
        )
    )
    assert result["success"] is True
    assert calls == ["ran-through-octopus"]
...
async def test_selected_codex_plugin_is_loaded_on_demand_without_ambient_surfaces(
    ...
    # The plugin remains installed in Octopus' registry, but removing the
