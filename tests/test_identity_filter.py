...
def _force_lock_on(monkeypatch: pytest.MonkeyPatch):
    ...
    ``OCTOPUS_IDENTITY_LOCK`` setting."""
    monkeypatch.setenv("OCTOPUS_IDENTITY_LOCK", "1")
...
class TestAgentIdentity:
    def test_non_octopus_agent_self_claim_is_rewritten(self):
        class Agent:
            agent_id = "coder"
            display_name = "Coder"

        out = identity_filter.filter_text("我是 Octopus, 可以帮你写代码。", agent=Agent())
        assert out.startswith("我是 Coder"), out
        assert "我是 Octopus" not in out

    def test_non_octopus_agent_english_self_claim_is_rewritten(self):
        class Agent:
            agent_id = "researcher"
            display_name = "Researcher"

        out = identity_filter.filter_text("I'm Octopus, ready to research.", agent=Agent())
        assert out.startswith("I'm Researcher"), out
        assert "I'm Octopus" not in out


class TestLockingOff:
    def test_no_rewrite_when_lock_off(self, monkeypatch: pytest.MonkeyPatch):
        """Developer mode · OCTOPUS_IDENTITY_LOCK=0 · filter is a no-op."""
        monkeypatch.setenv("OCTOPUS_IDENTITY_LOCK", "0")
        identity_filter.set_runtime_lock(None)
