...
def test_systemd_baseline_is_authenticated_production_and_loadable(
    ...
    monkeypatch.setenv("OCTOPUS_ADMIN_PASSWORD_HASH", "sha256:" + "a" * 64)
    ...
    assert config.journal_file == "/var/lib/octopus/events.jsonl"
...
def test_systemd_unit_uses_dedicated_baseline_and_persistent_data_root() -> None:
    service = (ROOT / "deploy/octopus-agent.service").read_text(encoding="utf-8")
    deployment = (ROOT / "docs/deployment.md").read_text(encoding="utf-8")

    assert "--config /etc/octopus/config.yaml" in service
    assert "--host 0.0.0.0" in service
    assert "Environment=OCTOPUS_DATA_DIR=/var/lib/octopus" in service
    assert "deploy/systemd-config.yaml" in service
    assert "deploy/systemd-config.yaml" in deployment
    assert "octopus-agent-runtime[serve,local-auth,anthropic]" in deployment
    assert "printf 'Octopus!9%s\\n' \"$(openssl rand -base64 48)\"" in deployment
    assert "local_auth.config import hash_password" in deployment
    assert "sudo cp config.example.yaml /etc/octopus/config.yaml" not in deployment


def test_compose_healthchecks_use_real_readiness_probe() -> None:
    for filename in ("docker-compose.yml", "docker-compose.full.yml"):
        compose = yaml.safe_load((ROOT / filename).read_text(encoding="utf-8"))
        command = " ".join(compose["services"]["octopus-agent"]["healthcheck"]["test"])
...
def test_k8s_manifest_binds_auth_redis_and_real_probe_contract() -> None:
    ...
    fail_closed_image = "ghcr.io/dengdenghua/octopus-agent@sha256:" + "0" * 64
    ...
    assert env["OCTOPUS_HEARTS_REDIS_URL"]["value"].startswith("redis://:")
...
def test_k8s_admin_login_can_reach_real_admin_control_plane(
    ...
    monkeypatch.setenv("OCTOPUS_ADMIN_PASSWORD_HASH", "sha256:" + password_hash)
    monkeypatch.setenv("REDIS_PASSWORD", "redis-test-password")
...
def test_k8s_mutable_resource_contract_supports_real_market_install(
    ...
    monkeypatch.setenv("OCTOPUS_RESOURCES_DIR", str(mutable_resources))
    monkeypatch.setattr(
        agent_world_router,
        "_INSTALL_STATE",
...
def test_k8s_network_policy_is_explicit_and_fail_closed() -> None:
    ...
    agent_ingress = by_name["octopus-agent-ingress"]["spec"]["ingress"][0]
    namespace_labels = agent_ingress["from"][0]["namespaceSelector"]["matchLabels"]
    assert namespace_labels == {"octopus-agent.io/ingress-access": "true"}
    redis_ingress = by_name["redis-ingress"]["spec"]["ingress"][0]
    assert redis_ingress["from"][0]["podSelector"]["matchLabels"] == {"app": "octopus-agent"}
...
def test_deployment_docs_describe_builtin_auth_and_probe_semantics() -> None:
    ...
    assert "octopus-agent.io/ingress-access=true" in deployment
