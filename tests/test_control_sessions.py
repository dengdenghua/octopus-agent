...
def test_control_session_replay_records_actions_and_evidence(tmp_path) -> None:
    ...
    assert data["timeline"]["schema"] == "octopus.control_session_replay_timeline.v1"
    ...
    assert timeline_body["schema"] == "octopus.control_session_replay_timeline.v1"
...
def test_control_session_timeline_pages_kimi_scale_swarm_replay(tmp_path) -> None:
    ...
    for idx in range(300):
        response = client.post(
            ...
            json={
                ...
                "detail": {
                    "schema": "octopus.swarm_replay_package.v1",
    ...
    for page_index in range(4):
        ...
        assert response.status_code == 200
        body = response.json()
        assert body["schema"] == "octopus.control_session_replay_timeline.v1"
...
def test_control_session_stores_large_swarm_replay_evidence_as_blob_ref(tmp_path) -> None:
    ...
    assert evidence_node["detail_schema"] == "octopus.control_evidence_blob_ref.v1"
    assert evidence_node["truncated"] is True
    assert detail["schema"] == "octopus.control_evidence_blob_ref.v1"
    ...
    with open(detail["path"], "rb") as fh:
        ...
        assert b"octopus.swarm_replay_package.v1" in blob
    ...
    assert body["schema"] == "octopus.control_evidence_detail.v1"
    assert body["source"] == "blob"
    assert body["detail"]["schema"] == "octopus.swarm_replay_package.v1"
