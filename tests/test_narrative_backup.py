...
def test_custom_data_dir_backup_restore_and_export_are_relocatable(
    ...
    monkeypatch.setenv("OCTOPUS_DATA_DIR", str(source_data))
    ...
    monkeypatch.setenv("OCTOPUS_DATA_DIR", str(destination_data))
...
def test_implicit_octopus_home_uses_home_data_directory(tmp_path: Path, monkeypatch) -> None:
    octopus_home = tmp_path / "custom-home"
    project_file = (
        octopus_home / "data" / "narrative-studio" / "projects" / "home-story" / "project.json"
    )
    project_file.parent.mkdir(parents=True)
...
def test_implicit_without_environment_keeps_dot_octopus_layout(
    tmp_path: Path,
    monkeypatch,
) -> None:
