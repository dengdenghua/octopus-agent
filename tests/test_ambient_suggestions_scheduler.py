...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    Points ``OCTOPUS_HOME`` at it so ``_project_root()`` resolves
    ...
    monkeypatch.delenv(
        "OCTOPUS_FF_UI_AMBIENT_SUGGESTIONS_INTERVAL_SEC", raising=False,
    )
    ff._SNAPSHOT = None
    return tmp_path
...
def _enable_flag(monkeypatch: pytest.MonkeyPatch, value: str = "1") -> None:
    monkeypatch.setenv("OCTOPUS_FF_UI_AMBIENT_SUGGESTIONS", value)
    ff._SNAPSHOT = None
...
def test_tick_once_returns_expected_keys(
    ...
    with patch(
        ...
        out = sched.tick_once()
    assert set(out.keys()) == {
        "agents_processed", "errors", "suggestions_added",
    }
...
def test_start_then_stop_returns_quickly(
    ...
    sched.start(AmbientSchedulerConfig(
        interval_sec=60,
...
def test_start_honors_initial_delay_no_tick_before_stop(
    ...
    with patch(
        ...
        sched.stop(timeout=2.0)
    assert gen.call_count == 0, (
        "generate_suggestions must not be called before initial_delay"
    )
...
def test_interval_flag_env_resolves(
    ...
    monkeypatch.setenv(
        "OCTOPUS_FF_UI_AMBIENT_SUGGESTIONS_INTERVAL_SEC", "900",
    )
    ff._SNAPSHOT = None
    sched = get_ambient_scheduler()
    ...
    monkeypatch.setenv(
        "OCTOPUS_FF_UI_AMBIENT_SUGGESTIONS_INTERVAL_SEC", "5",
    )
    ff._SNAPSHOT = None
...
def test_does_not_crash_on_malformed_scores_lines(
    ...
    _enable_flag(monkeypatch)
    _write_scores(
        workspace, "coder", turn_count=5, include_malformed=True,
    )
    sched = get_ambient_scheduler()
    with patch(
        "runtime.memory.skills_lib.ambient_suggestions.generate_suggestions",
