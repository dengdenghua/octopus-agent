...
def test_persistent_store_keeps_only_latest_state_in_memory_but_serves_history(
    ...
    assert store._history["thread-1"][0]["values"] is store._threads["thread-1"]["values"]
    path = tmp_path / "agents" / "general" / "sessions" / "thread-1.jsonl"
    ...
    assert [state["values"]["title"] for state in store.get_history("thread-1", limit=3)] == [
        "title-4",
        "title-3",
        "title-2",
    ]
    ...
    assert [state["values"]["title"] for state in reloaded.get_history("thread-1", limit=3)] == [
        "title-4",
        "title-3",
        "title-2",
    ]
