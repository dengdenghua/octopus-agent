...
def test_evolution_dataset_clusters_verification_by_repair_route() -> None:
    ...
    primary_examples = [
        example for example in dataset.all_examples if not example.source.endswith(":synthetic")
    ]
    assert all("Use repair route" in example.expected_behavior for example in primary_examples)
