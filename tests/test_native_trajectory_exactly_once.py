...
def test_jsonl_atomic_trajectory_uses_pseudonymized_pii_scope_as_dedupe_key(
    ...
    stored = journal.read_by_type("trajectory")
    assert len(stored) == 2
    assert all(event.tenant_id.startswith("octopus-scope-tenant-vone-") for event in stored)
    assert all(event.owner_actor_id.startswith("octopus-scope-owner-vone-") for event in stored)
