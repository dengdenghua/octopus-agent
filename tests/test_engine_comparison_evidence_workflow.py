...
def test_engine_evidence_requires_protected_preprovisioned_linux_runner() -> None:
    ...
    assert job["env"]["OCTOPUS_HARDENED_VERIFIER_RUNNER"] == (
        "${{ vars.OCTOPUS_HARDENED_ATTESTATION_PATH }}"
    )
    preflight = _step("Fail closed unless protected runner identities are complete")["run"]
    assert "OCTOPUS_ENGINE_EVIDENCE_GUARD" in preflight
    assert "root-owned and non-writable" in preflight
    assert '"${OCTOPUS_HARDENED_LAUNCHER_PYTHON}" -I' in preflight
    assert '"${OCTOPUS_HARDENED_LAUNCHER_MODULE}" validate' in preflight
    assert "OCTOPUS_HARDENED_LAUNCHER_PYTHON_SHA256" in preflight
    assert "OCTOPUS_HARDENED_LAUNCHER_MODULE_SHA256" in preflight
    ...
    assert '.candidate_api_isolation_schema == "octopus.candidate_api_process.v1"' in preflight
...
def test_attested_sources_are_bound_before_attack_or_model_execution() -> None:
    ...
    start_name = "Start one isolated Octopus control plane"
...
def test_engine_evidence_uses_same_service_and_seeded_ab_ba_schedule() -> None:
    start = _step("Start one isolated Octopus control plane")["run"]
...
def test_real_linux_attack_suite_precedes_service_and_is_provenance_bound() -> None:
    ...
    assert attack["env"] == {"OCTOPUS_RUN_HARDENED_VERIFIER_ATTACKS": "1"}
    ...
    assert "OCTOPUS_API_TOKEN" not in str(attack)
...
def test_protected_runtime_inputs_are_rechecked_without_publishing_auth_identity() -> None:
    ...
    for value in (
        "OCTOPUS_EVAL_CONFIG",
        "OCTOPUS_CODEX_EXECUTABLE",
        "OCTOPUS_CODEX_SOURCE_HOME",
        "auth_path",
    ):
    ...
    assert "OCTOPUS_EVAL_EXPECTED_CONFIG_SHA256" in revalidate["run"]
    assert "OCTOPUS_EVAL_EXPECTED_CODEX_SHA256" in revalidate["run"]
    ...
    assert (
        "OCTOPUS_PROTECTED_IDENTITY_BASELINE"
        in _step("Stop the isolated Octopus control plane")["run"]
    )
