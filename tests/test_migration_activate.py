...
Hermetic — stages a fake ``.octopus/imported/`` under tmp_path and activates,
...
def test_activate_memory_indexes_into_project_memory_idempotently(tmp_path: Path) -> None:
    ...
    index = (tmp_path / ".octopus" / "MEMORY.md").read_text(encoding="utf-8")
    assert "[codex] pref: likes tests" in index
    assert ".octopus/imported/codex/memory/pref.md" in index  # pointer to full text
...
def test_run_migrate_preview_returns_zero(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / ".octopus").exists()  # preview writes nothing
