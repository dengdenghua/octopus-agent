...
def test_proxy_is_mounted_by_default_without_environment(monkeypatch) -> None:
    monkeypatch.delenv("OCTOPUS_MX2025_PROXY_ORIGIN", raising=False)
    monkeypatch.delenv("OCTOPUS_MX2025_ALLOW_SAME_ORIGIN_SCRIPTS", raising=False)
...
def test_explicit_config_wins_over_enable_environment(
    ...
    monkeypatch.setenv("OCTOPUS_MX2025_PROXY_ORIGIN", "true")
    monkeypatch.setenv("OCTOPUS_MX2025_ALLOW_SAME_ORIGIN_SCRIPTS", "true")
    config: dict[str, object] = {
        "proxy_origin": True,
        "allow_same_origin_third_party_scripts": True,
        "base_url": "https://up.test",
...
def test_local_explicit_double_switch_keeps_viewer_compatible() -> None:
    ...
    assert "?octopus_proxy=7#/" in page.text
...
def test_rewrites_root_relative_spa_assets_and_api_calls() -> None:
    ...
    assert f"{prefix}/assets/app.js?octopus_proxy=7" in rewritten
    ...
    assert f'"{prefix.lstrip("/")}/assets/login.js?octopus_proxy=7"' in rewritten_script
    assert '"./pages-login.js?octopus_proxy=7"' in rewritten_script
