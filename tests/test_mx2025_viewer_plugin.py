...
def _load_plugin(
    ...
    *,
    ...
    app = FastAPI()
    app.state.octopus_require_auth = authenticated_host
...
def test_bundled_manifest_is_loadable_and_fail_closed(tmp_path: Path) -> None:
    hub = PluginHub(
        plugin_dir=tmp_path,
    ...
    assert schema["proxy_origin"]["default"] is False
    assert schema["allow_same_origin_third_party_scripts"]["default"] is False
    assert [cap["name"] for cap in listed["capabilities"]] == ["mx2025_viewer.api"]


def test_proxy_is_disabled_by_default_and_page_is_inert() -> None:
    ...
    assert "同源代理未开启" in page.text
    assert "<iframe" not in page.text
    assert "default-src 'none'" in page.headers["content-security-policy"]
    assert client.get(f"/api/plugins/{PLUGIN_ID}/origin/").status_code == 404


@pytest.mark.parametrize(
    "config",
    [
        {"proxy_origin": True, "base_url": "https://up.test"},
        {
            "proxy_origin": True,
...
def test_proxy_requires_two_real_booleans_and_https(config: dict[str, object]) -> None:
...
def test_proxy_strips_host_credentials_and_upstream_cookie_and_sets_csp() -> None:
    ...
    response = TestClient(_proxy_app(upstream)).get(
        ...
        headers={
            "Authorization": "Bearer octopus-host-token",
            "Cookie": "octopus_session=host-secret",
