...
def _trusted_full_chain_smoke() -> dict[str, Any]:
    return {
        "schema": "octopus.hardened_verifier_full_chain_smoke.v1",
        "passed": True,
        "cases": [
...
@pytest.mark.parametrize(
    ("field", "value", "message"),
    [
        ("schema", "octopus.hardened_verifier_runner.v1", "unsupported schema"),
...
def test_comparison_artifact_records_fixture_runner_provenance(
    ...
    provenance = {
        "schema": "octopus.fixture_test_runner.v1",
        "ownership": "evaluator",
        "interpreter_path": "/controlled/python",
        "python_version": "3.12.0",
        "pytest_version": "9.0.0",
        "runner_path": ".octopus-eval/run-tests",
        "runner_sha256": "a" * 64,
        "test_root": "tests",
    }
    verifier_provenance = {
        "schema": "octopus.hardened_verifier_runner.v1",
            ...
            observed.append(
                {
                    ...
                    "workspace": workspace,
                    "context": context,
                    "runner_exists": (workspace / ".octopus-eval/run-tests").is_file()
                    or (workspace / ".octopus-eval/run-tests.cmd").is_file(),
...
def test_native_approval_is_exact_path_and_command_allowlist(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert responder(
            "item/commandExecution/requestApproval",
            {"tool": "exec_shell", "argsPreview": "{'command': './.octopus-eval/run-tests'}"},
        ) == {"action": "accept"}
        ...
        denied = [
            ...
            {
                "tool": "exec_shell",
                "argsPreview": "{'command': './.octopus-eval/run-tests', 'cwd': '/tmp'}",
            },
...
def test_comparison_payload_keeps_raw_measurements_without_synthetic_score() -> None:
    ...
    assert payload["schema"] == "octopus.engine_comparison.v2"
