...
builtins.OCTOPUS_CANDIDATE_SENTINEL = "child-only"
...
"""
    ...
    assert not hasattr(builtins, "OCTOPUS_CANDIDATE_SENTINEL")
...
def test_candidate_api_schema_is_content_bound_across_all_trusted_layers() -> None:
    assert CANDIDATE_API_ISOLATION_SCHEMA == "octopus.candidate_api_process.v1"
...
def test_missing_runner_contract_exits_78_before_candidate_execution(
    ...
    workspace = _path_workspace(tmp_path)
    for name in (
        "OCTOPUS_CANDIDATE_WORKER_CONFIG",
        "OCTOPUS_CANDIDATE_WORKER_CONFIG_SHA256",
        "OCTOPUS_CANDIDATE_WORKER_CLI_JSON",
    ):
...
def test_absolute_controller_rejects_wrong_contract_digest_as_infrastructure(
    ...
    completed = subprocess.run(
        ...
        text=True,
        env={
            "OCTOPUS_CANDIDATE_WORKER_CONTRACT_SHA256": "0" * 64,
