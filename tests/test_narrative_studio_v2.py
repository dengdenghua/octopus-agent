...
def test_v1_project_is_atomically_migrated_and_future_fields_survive(tmp_path: Path) -> None:
    ...
    old = {
        "schema_version": "octopus.narrative-studio.project.v1",
        "id": "legacy",
        "title": "Legacy",
    ...
    assert migrated.schema_version == "octopus.narrative-studio.project.v2"
    assert migrated.migrated_from == "octopus.narrative-studio.project.v1"
    assert migrated.governance.review_quorum == 2
    assert persisted["schema_version"] == "octopus.narrative-studio.project.v2"
