...
def test_kimi_swarm_certification_is_evidence_backed(tmp_path) -> None:
    ...
    assert report["schema"] == "octopus.kimi_swarm_certification.v1"
    assert report["ready"] is True
    assert report["score"] == 1.0
    assert report["passed"] == report["total"] == 9
    assert report["verdict"] == "deterministic_orchestration_surpassed"
    assert report["summary"]["octopus_deterministic_max_members"] == 320
    assert report["summary"]["octopus_worker_concurrency_proven"] == 32
