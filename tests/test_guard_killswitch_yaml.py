...
The persistent layer companion to OCTOPUS_DISABLED_GUARDS env var:
...
class TestYamlReader:
    ...
    def test_yaml_with_safety_block(self, isolated_cwd: Path) -> None:
        _write_yaml(isolated_cwd / "config.local.yaml", """
safety:
  disabled_guards:
    - magic-number guard
    - long-function guard
...
""")
        _write_yaml(isolated_cwd / "config.example.yaml", """
safety:
...
""")
    ...
    def test_malformed_yaml_silent(self, isolated_cwd: Path) -> None:
        _write_yaml(
            isolated_cwd / "config.local.yaml", "{this is not: valid: yaml: at all",
        )
...
""")
    ...
    def test_env_only(
        ...
        monkeypatch.setenv("OCTOPUS_DISABLED_GUARDS", "from-env")
...
""")
        monkeypatch.setenv(
            "OCTOPUS_DISABLED_GUARDS", "from-env-1,from-env-2",
        )
        assert _disabled_guard_labels() == frozenset({
            "from-yaml-1", "from-yaml-2",
...
""")
        monkeypatch.setenv("OCTOPUS_DISABLED_GUARDS", "shared-guard")
    ...
    def test_yaml_broken_env_works(
        ...
        _write_yaml(
            isolated_cwd / "config.local.yaml", "{garbage: not: valid",
        )
        monkeypatch.setenv("OCTOPUS_DISABLED_GUARDS", "from-env")
        assert _disabled_guard_labels() == frozenset({"from-env"})
