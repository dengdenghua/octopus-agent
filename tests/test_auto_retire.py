...
class _SpyRouter(ModelRouter):
    ...
    def call(self, request: ModelRequest) -> ModelResponse:
        ...
        return ModelResponse(
            ...
            cost=CostEntry(usd=0.001), model=request.model, provider="mock",
        )
    ...
    for _ in range(failures):
        ...
        step = Step(
            ...
            result=ExecutionResult(
                call_id=call.call_id, status="failed", error_type="timeout",
            ),
        )
        stack.journal.write_trajectory(Trajectory(
            task_id=TaskId(uuid4()), arm_id=ArmId("a"),
...
def _make_evolver_with_losers(stack) -> PromptEvolver:
    ...
    return PromptEvolver(
        opt, mutator,
        EvolutionPolicy(
            retire_min_uses=5, mutate_each_step=False,
...
class TestTickGuards:
    def test_first_tick_blocked_by_interval(self, stack):
        ...
        sched = AutoRetireScheduler(evolver, config=AutoRetireConfig(
    ...
    def test_too_few_variants_blocks(self, stack):
        ...
        evolver = PromptEvolver(opt, mutator, EvolutionPolicy(
            mutate_each_step=False, crossover_each_step=False,
        ))
        sched = AutoRetireScheduler(evolver, config=AutoRetireConfig(
            min_interval_seconds=0.0,
