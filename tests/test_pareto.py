...
class TestParetoAlgorithm:
    def test_empty_input(self):
        assert pareto_frontier_by_name(
            {}, ("success",), maximize={"success": True},
        ) == set()

    def test_single_point_on_frontier(self):
        points = {"only": {"success": 0.5}}
        out = pareto_frontier_by_name(
            points, ("success",), maximize={"success": True},
        )
        assert out == {"only"}
    ...
    def test_2d_frontier_covers_tradeoff(self):
        ...
        out = pareto_frontier_by_name(
            points, ("s", "c"), maximize={"s": True, "c": False},
        )
...
class TestEvolverParetoMode:
    def test_pareto_keeps_cheap_variant_even_with_lower_success(self, stack):
        ...
        _seed_outcomes(stack, opt.planner_for("expensive").recipe_hash(),
                       successes=9, failures=1, cost_per_task=0.05)
        _seed_outcomes(stack, opt.planner_for("cheap").recipe_hash(),
                       successes=6, failures=4, cost_per_task=0.001)
        _seed_outcomes(stack, opt.planner_for("dominated").recipe_hash(),
                       successes=5, failures=5, cost_per_task=0.05)
        for n in ("expensive", "cheap", "dominated"):
            opt._splitter.stats[n].assignments = 10

        mutator = PromptMutator(
            router=_StubRouter("<suffix>x</suffix>"), model="m",
        )
    ...
    def test_verdict_mode_would_clear_cheap_but_pareto_saves_it(self, stack):
        ...
        _seed_outcomes(stack, opt.planner_for("expensive").recipe_hash(),
                       successes=9, failures=1, cost_per_task=0.05)
        _seed_outcomes(stack, opt.planner_for("cheap").recipe_hash(),
                       successes=2, failures=8, cost_per_task=0.001)
        ...
        # Implementation note.
        ev_verdict = PromptEvolver(opt, mutator, EvolutionPolicy(
            use_pareto=False,
            retire_min_uses=5,
            min_variants_after_retire=1,
            mutate_each_step=False,
    ...
    def test_pareto_mode_skips_when_data_insufficient(self, stack):
        ...
        # Implementation note.
        _seed_outcomes(stack, opt.planner_for("a").recipe_hash(),
