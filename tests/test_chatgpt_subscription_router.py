...
def test_router_retries_clean_remote_disconnect_before_any_output() -> None:
    calls = 0
    ...
    response = router.call(
        ModelRequest(model="chatgpt/gpt-5.6-sol", messages=[Message(role="user", content="hi")])
    )
...
def test_router_does_not_replay_after_partial_output() -> None:
    class _BrokenStream(httpx.SyncByteStream):
        def __iter__(self):
            yield _sse({"type": "response.output_text.delta", "delta": "半"})
    ...
    stream = router.call_stream(
        ModelRequest(model="chatgpt/gpt-5.6-sol", messages=[Message(role="user", content="hi")])
    )
    first = next(stream)
    assert first.delta == "半"
    with pytest.raises(httpx.RemoteProtocolError):
        list(stream)
    assert calls == 1


def test_dispatch_prefix_keeps_octopus_kernel_and_selects_chatgpt_transport() -> None:
