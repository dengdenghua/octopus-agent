"""File Agent document search — best-effort octopus-storage client + skill."""
...
def test_base_url_env_override(monkeypatch) -> None:
    monkeypatch.setenv("OCTOPUS_STORAGE_URL", "http://host:9000/")
...
def test_search_storage_unavailable_is_actionable(monkeypatch) -> None:
    ...
    assert "octopus-storage" in r["message"]  # tells the user how to enable it
    assert r["hits"] == []
