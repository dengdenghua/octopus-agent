...
class TestNotebookRead:
    ...
    def test_code_outputs_text_preserved(self, tmp_path: Path):
        p = tmp_path / "x.ipynb"
        _write_nb(p, [
            {"cell_type": "code", "id": "c1", "source": "print(42)",
    ...
    def test_binary_output_dropped(self, tmp_path: Path):
        ...
        p = tmp_path / "x.ipynb"
        _write_nb(p, [
            {"cell_type": "code", "id": "c1", "source": "plot()",
             "metadata": {}, "execution_count": 1,
             "outputs": [
                 {"output_type": "display_data",
                  "data": {"image/png": "AAAA" * 1000}},
             ]},
        ])
        r = _notebook_read(path=str(p))
...
class TestNotebookEdit:
    def test_append_new_cell(self, tmp_path: Path):
        p = tmp_path / "x.ipynb"
        _write_nb(p, [
            {"cell_type": "code", "id": "c1", "source": "a=1",
             "metadata": {}, "outputs": [], "execution_count": None},
        ])
        r = _notebook_edit(
            path=str(p), mode="append",
            new_source="# new section", cell_type="markdown",
        )
        assert r.get("ok")
        assert r["cell_count"] == 2
        # Verify persisted
    ...
    def test_replace_by_index(self, tmp_path: Path):
        p = tmp_path / "x.ipynb"
        _write_nb(p, [
            {"cell_type": "code", "id": "c1", "source": "old",
             "metadata": {}, "outputs": [], "execution_count": None},
            {"cell_type": "code", "id": "c2", "source": "keep",
             "metadata": {}, "outputs": [], "execution_count": None},
        ])
        r = _notebook_edit(
            path=str(p), mode="replace",
            cell_index=0, new_source="replaced", cell_type="code",
        )
        assert r.get("ok")
        nb = json.loads(p.read_text(encoding="utf-8"))
    ...
    def test_replace_by_id(self, tmp_path: Path):
        p = tmp_path / "x.ipynb"
        _write_nb(p, [
            {"cell_type": "code", "id": "first", "source": "a",
             "metadata": {}, "outputs": [], "execution_count": None},
            {"cell_type": "code", "id": "target", "source": "old",
             "metadata": {}, "outputs": [], "execution_count": None},
        ])
        r = _notebook_edit(
            path=str(p), mode="replace",
            cell_id="target", new_source="NEW", cell_type="code",
        )
        assert r.get("ok")
        nb = json.loads(p.read_text(encoding="utf-8"))
    ...
    def test_insert_after_index(self, tmp_path: Path):
        ...
        _write_nb(p, [
            {"cell_type": "code", "id": "c1", "source": "A",
             "metadata": {}, "outputs": [], "execution_count": None},
            {"cell_type": "code", "id": "c2", "source": "B",
             "metadata": {}, "outputs": [], "execution_count": None},
        ])
        _notebook_edit(
            path=str(p), mode="insert",
            cell_index=0, new_source="MID", cell_type="markdown",
        )
        nb = json.loads(p.read_text(encoding="utf-8"))
    ...
    def test_missing_cell_returns_error(self, tmp_path: Path):
        ...
        r = _notebook_edit(
            path=str(p), mode="replace",
            cell_id="does_not_exist", new_source="y",
        )
        assert "error" in r
