...
def registry_large() -> SkillRegistry:
    ...
    return _mk_registry(
        ...
        "web_search", "rag_lookup", "summarize",
        "exec_shell", "git_commit", "git_log",
        "hash_text",
    )


def _mk_arm(
    arm_id: str, *, affinity: list[str], skills: list[str],
) -> Worker:
...
class TestPickForIntent:
    def test_affinity_keyword_match(self):
        code_arm = _mk_arm(
            "code_arm", affinity=["code", "git"],
            skills=["read_file", "git_commit"],
        )
        search_arm = _mk_arm(
            "search_arm", affinity=["search", "web"],
        ...
        # Implementation note.
        intent = ParsedIntent(
            raw="commit the changes", intent_type="task",
        ...
        intent2 = ParsedIntent(
            raw="go search the web", intent_type="task",
    ...
    def test_empty_pool(self):
        pool = ArmPool([])
        intent = ParsedIntent(
            raw="x", intent_type="task", normalized_goal="anything",
        )
        assert pool.pick_for_intent(intent) is None
...
class TestLLMPlannerScopedContext:
    def _make_planner(self, registry: SkillRegistry, router: MockModelRouter):
        ...
        return LLMPlanner(
            router=router, registry=registry, composer=composer,
        )
    ...
    def test_default_still_injects_all(self, registry_large):
        ...
        planner.plan(intent)
        sucker_text = "\n".join(
            m.content for m in router.call_log[0].messages
            if m.role != "system"
        )
        # Implementation note.
    ...
    def test_token_savings_observable(self, registry_large):
        ...
        router_full = MockModelRouter(
            response=json.dumps({
                "reasoning": "", "nodes": [{"skill": "read_file", "args": {}}],
            }),
        )
        ...
        intent = ParsedIntent(
            raw="x", intent_type="task", normalized_goal="do stuff",
        )
        p_full.plan(intent)
        p_scoped.plan(intent, allowed_skills=["read_file"])

        full_len = sum(
            len(m.content) for m in router_full.call_log[0].messages
        )
        scoped_len = sum(
            len(m.content) for m in router_scoped.call_log[0].messages
        )
...
class TestStaticPlannerScoped:
    def test_rule_skipped_when_skill_not_allowed(self):
        ...
        intent = ParsedIntent(
            raw="x", intent_type="task", normalized_goal="x",
        )
        # Implementation note.
...
class TestEndToEnd:
    def test_full_per_role_flow(self, registry_large):
        ...
        planner = LLMPlanner(
            router=router, registry=registry_large, composer=composer,
        )
