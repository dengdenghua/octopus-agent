...
def test_systemd_unit_is_loopback_only_hardened_and_release_scoped() -> None:
    ...
    assert "User=octopus-quote" in unit
    assert "Group=octopus-quote" in unit
    assert "WorkingDirectory=/opt/octopus-cloud/quote-hub/current" in unit
    assert "EnvironmentFile=/etc/octopus/quote-hub.env" in unit
    assert "/opt/octopus-cloud/quote-hub/current/.venv/bin/python -m uvicorn" in unit
    ...
    assert "ReadWritePaths=/var/lib/octopus/quote-hub" in unit
...
def test_environment_and_secret_examples_have_placeholders_only() -> None:
    ...
    assert "QUOTE_HUB_SECRET_FILE=/etc/octopus/quote-hub-secret.json" in env
...
def test_release_scripts_are_executable_syntax_checked_and_atomic() -> None:
    ...
    for name in ("deploy-release.sh", "rollback-release.sh"):
        script = _read(name)
        assert "BASE_DIR=/opt/octopus-cloud/quote-hub" in script
...
def test_deployment_guide_documents_permissions_auth_and_rollback() -> None:
    ...
    assert "octopus-quote:octopus-quote" in guide
