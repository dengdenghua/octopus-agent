...
Fernet key via ``OCTOPUS_WORKSPACE_KEY`` so encryption behavior is
...
def _reset_crypto_cache() -> None:
    ...
    per-test env-var changes (``OCTOPUS_WORKSPACE_KEY``) take effect cleanly
...
def crypto_enabled(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("OCTOPUS_WORKSPACE_KEY", key)
