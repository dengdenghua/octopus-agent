...
class TestConstruct:
    def test_requires_app_id(self):
        with pytest.raises(ValueError, match="app_id"):
            FeishuChannel(
                app_id="", app_secret="s", verification_token="v",
            )

    def test_requires_app_secret(self):
        with pytest.raises(ValueError, match="app_secret"):
            FeishuChannel(
                app_id="cli_x", app_secret="", verification_token="v",
            )

    def test_requires_verification_token(self):
        with pytest.raises(ValueError, match="verification_token"):
            FeishuChannel(
                app_id="cli_x", app_secret="s", verification_token="",
            )

    def test_default_channel_id(self):
        ch = FeishuChannel(
            app_id="cli_x", app_secret="s", verification_token="v",
        )
...
class TestURLVerification:
    def test_challenge_returned_when_token_matches(self):
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="TOKEN",
        )
        body = json.dumps({
...
class TestEventHandling:
    ...
    def test_bad_token_raises(self):
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="RIGHT",
        )
    ...
    def test_bad_json_body_raises(self):
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="T",
        )
...
class TestSend:
    def test_send_acquires_token_then_posts(self):
        ...
        ch = FeishuChannel(
            app_id="cli_x", app_secret="s", verification_token="T",
            http_client=http,
        )
    ...
    def test_token_cached_between_sends(self):
        ...
        for _ in range(3):
            ch.send(OutboundMessage(
                channel_id="feishu", thread_id="c:m",
                content="x",
    ...
    def test_receive_id_from_metadata_preferred(self):
        ...
        ch.send(OutboundMessage(
            channel_id="feishu", thread_id="fallback:x",
            content="y",
    ...
    def test_send_api_error_raises(self):
        http = _RouteHttp({
            "/auth/v3/tenant_access_token": _token_ok_resp("x"),
            "/im/v1/messages": _FakeResp(body={
                "code": 230002, "msg": "chat not found",
            }),
        })
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="T",
        ...
        with pytest.raises(FeishuError, match="230002"):
            ch.send(OutboundMessage(
                channel_id="feishu", thread_id="c:m", content="x",
                metadata={"receive_id": "c"},
            ))

    def test_token_fetch_api_error_raises(self):
        http = _RouteHttp({
            "/auth/v3/tenant_access_token": _FakeResp(body={
                "code": 99991663,
                "msg": "app_id invalid",
            }),
        })
        ch = FeishuChannel(
            app_id="cli_bad", app_secret="s", verification_token="T",
            http_client=http,
        )
    ...
    def test_http_500_raises(self):
        http = _RouteHttp({
            "/auth/v3/tenant_access_token": _FakeResp(
                status_code=500, text="upstream down",
            ),
        })
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="T",
...
class TestE2E:
    def test_inbound_produces_valid_message(self):
        ch = FeishuChannel(
            app_id="a", app_secret="s", verification_token="TOK",
        )
        body = json.dumps({
            "schema": "2.0",
            "header": {
