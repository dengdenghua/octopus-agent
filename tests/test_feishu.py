...
class TestEdit:
    def test_edit_patches_message_with_contract_param_order(self):
        # Regression: edit() previously declared (original_message_id, msg),
        # but Channel.edit (base.py) + manager.py call it positionally as
        # (msg, original_message_id). The inversion made feishu PATCH the
        # message-id slot with an OutboundMessage repr and crash on
        # str.content. Pin the contract order here.
        http = _RouteHttp(
            {
                "/auth/v3/tenant_access_token": _token_ok_resp("my-tat"),
                "/im/v1/messages": _send_ok_resp(),
        ...
        msg = OutboundMessage(
            channel_id="feishu",
            thread_id="oc_chat:om_root",
            content="改后的内容",
        )
        ch.edit(msg, "om_message123")
        # token fetch + patch
        assert len(http.calls) == 2
        patch_call = http.calls[1]
        # original_message_id lands in the URL (not the OutboundMessage repr)
        assert patch_call["url"].endswith("/im/v1/messages/om_message123")
        # msg.content lands in the body
        content_str = patch_call["json"]["content"]
        assert json.loads(content_str)["text"] == "改后的内容"
