...
- [ ] 单测覆盖上述两场景并通过（timeline-role.test.ts 12 用例已编写；终端故障 code:5999 未执行，见 Task 7）
...
- [ ] 可解析/不可解析/空结果三分支单测通过（fact-summary.test.ts 19 用例已编写；待执行，见 Task 7）
...
- [ ] 联动 reducer/高亮生命周期有测试覆盖（timeline-linkage.test.ts 19 用例已编写；待执行，见 Task 7）

## 进展面板叙事大纲（Task 4）

- [x] 进展面板按 iteration 分组展示：每轮意图摘要一行 + 执行计数 + 事实列表
- [x] 分组可折叠（shadcn Collapsible），默认仅展开最近一轮
- [ ] ≥3 轮场景分组与默认折叠态单测通过；i18n 四语言（progress-outline.test.ts 9 用例 + agent-workbench-panel.test.tsx 4 用例已编写；i18n roundTitle/roundActionCount × 4 语言已核对；测试待执行，见 Task 7）
- [x] 进展/上下文面板不默认自动折叠（既有硬约束不破环）

## 紧凑模式叙事保真（Task 5）

- [x] `selectCompactTimelineItems` 语义感知：每个 iteration 必留 ≥1 个 intent 条目、最新 fact 条目必留
- [ ] 长任务压缩后单测通过，时间线仍可读「意图→执行→确认」节奏（测试已编写；终端故障待执行，见 Task 7）
