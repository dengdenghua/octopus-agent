...
| [immunity.md](immunity.md) | ③ 内生安全 | 4 层判决（Tolerance/Innate/Memory/Adaptive）+ 在线学习闭环 |
| [evolution.md](evolution.md) | ④ 进化（行为层）| 3 条回路（正向/负向/策略）+ 夜间 Batch + Shadow→Canary→Public |
| [distribution.md](distribution.md) | ⑤ Edge+Cloud | 三维度路由（latency/privacy/compute）+ 降级回退 + 跨 tier 消息分级 |
...
| [conflict_resolution.md](conflict_resolution.md) | **★ 冲突消解底座** | 6 类冲突 × 6 策略 × 固定优先级 + Trust EMA + 推理 trust 上限 |
