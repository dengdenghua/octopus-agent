...
# owner/repo pair ("octopus-agent" + "octopus-agent") and bare "/octopus"
# placeholders — both of which have appeared historically in the project.
# External links to other projects (e.g. "github.com/next.js/next.js" in
# attribution pages) are NOT flagged.
GITHUB_URL_RE = re.compile(
    r"https?://(?:www\.)?github\.com/"
    r"(?P<owner>dengdenghua|octopus(?:-ai|-agent)?|octopus-agent|octopus-ai)"
...
def _load_canonical() -> str:
    ...
    try:
        return str(data["tool"]["octopus"]["repo_url"])
    except KeyError as exc:
        raise SystemExit(
            "pyproject.toml is missing [tool.octopus] repo_url. Add it before running this linter."
        ) from exc
...
def main() -> int:
    ...
    if args.print_allowlist:
        print("Source of truth (pyproject.toml [tool.octopus].repo_url):")
        print(f"  {canonical}")
