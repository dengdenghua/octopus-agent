...
def ensure_repo(repo):
    ...
    gh(["repo", "create", repo, "--public", "--description",
        "🐙 Octopus 专家商城 — WorkBuddy(腾讯)领域专家/专家团 全量镜像(421 位)"])
...
def publish_storefront(repo):
    ...
    with tempfile.TemporaryDirectory(prefix="store-pub-") as tmp:
        ...
        sh(["git", "config", "user.email", "octopus-bot@users.noreply.github.com"], cwd=tmpd)
        sh(["git", "config", "user.name", "octopus-bot"], cwd=tmpd)
