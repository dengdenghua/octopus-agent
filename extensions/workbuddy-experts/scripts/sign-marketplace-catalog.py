...
def _private_key() -> Ed25519PrivateKey:
    encoded = os.environ.get("OCTOPUS_PLUGIN_SIGNING_PRIVATE_KEY", "").strip()
    if not encoded:
        raise ValueError("OCTOPUS_PLUGIN_SIGNING_PRIVATE_KEY is required")
    try:
...
def sign(path: Path) -> Path:
    ...
    publisher_id = os.environ.get(
        "OCTOPUS_PLUGIN_SIGNING_PUBLISHER_ID",
        "echoai",
    ).strip()
