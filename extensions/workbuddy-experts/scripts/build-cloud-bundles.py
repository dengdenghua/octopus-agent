...
def _sign_package(
    ...
    *,
    ...
    encoded_key = os.environ.get("OCTOPUS_PLUGIN_SIGNING_PRIVATE_KEY", "").strip()
    if not encoded_key:
        return False
    publisher_id = os.environ.get("OCTOPUS_PLUGIN_SIGNING_PUBLISHER_ID", "echoai").strip()
    key_id = os.environ.get("OCTOPUS_PLUGIN_SIGNING_KEY_ID", "").strip()
    if not publisher_id or not key_id:
        raise RuntimeError(
            "OCTOPUS_PLUGIN_SIGNING_PUBLISHER_ID and OCTOPUS_PLUGIN_SIGNING_KEY_ID "
    ...
    try:
        ...
        raise RuntimeError(
            "OCTOPUS_PLUGIN_SIGNING_PRIVATE_KEY must be a base64 Ed25519 private key"
        ) from exc
...
def build_plugins(out: Path) -> int:
    ...
    with tarfile.open(out / "octopus-plugins.tar.gz", "w:gz") as tf:
        ...
        with tempfile.TemporaryDirectory(prefix="octopus-marketplace-pack-") as tmp:
            stage_root = Path(tmp)
            seen: set[str] = set()
            source_roots = [REPO_CODEX_PLUGINS, REPO_OCTOPUS_PLUGINS]
            ...
            for source_root in source_roots:
                ...
                for manifest_path in sorted(source_root.glob("*/.codex-plugin/plugin.json")):
                    ...
                    staged_meta["octopus"] = derive_codex_package_requirements(
                        staged_meta,
                        package_dir=stage,
                    )
        ...
        if WORKBENCH_ROOT.exists():
            with tempfile.TemporaryDirectory(prefix="octopus-workbench-pack-") as tmp:
                stage_root = Path(tmp)
...
def main() -> None:
    ...
    if args.kind in {"all", "skills"}:
        files.append("octopus-skills.tar.gz")
    if args.kind in {"all", "plugins"}:
        files.append("octopus-plugins.tar.gz")
    for f in files:
        p = out / f
