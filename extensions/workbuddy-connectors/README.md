# WorkBuddy 连接器 → octopus 插件(Fork + 认证编排层)

> 从 WorkBuddy 5.3.14(腾讯)解包 fork 的**108 个连接器** + octopus **认证编排层**实现。
...
├── icons/                           # 连接器图标
├── octopus-manifest.json            # 规范化索引(转插件后,供注册表/商城消费)
├── INDEX.md                         # 可读索引
└── scripts/
    └── port-connectors-to-octopus.py  # 转成插件(packs/<id>/)脚本
```
...
| 模块 | 作用 |
|---|---|
| `credential_store.py` | AES-256-GCM 加密凭据库(`~/.octopus/connectors/`,明文永不落盘) |
...
## 转成我们的插件
```bash
python3 extensions/workbuddy-connectors/scripts/port-connectors-to-octopus.py          # manifest + INDEX
python3 extensions/workbuddy-connectors/scripts/port-connectors-to-octopus.py --packs  # 生成 packs/<id>/ 插件包
```
...
auth 注入 + 捆绑技能」。本仓库的认证编排层把 WorkBuddy 这套落地到 octopus 自身上。
