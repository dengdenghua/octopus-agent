(() => {
  ...
  const INDICATOR_ID = "octopus-browser-control-indicator";
  const STYLE_ID = "octopus-browser-control-indicator-style";
...
#${INDICATOR_ID}[data-state="action"] {
  animation: octopus-control-edge-pulse 620ms ease-out 1;
}
#${INDICATOR_ID}[data-state="paused"] {
...
@keyframes octopus-control-edge-pulse {
  0% {
    box-shadow:
...
`;
  ...
  if (typeof chrome !== "undefined" && chrome.runtime?.onMessage) {
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      if (message?.type === "octopus.controlIndicator") {
