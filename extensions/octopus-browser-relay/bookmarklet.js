(() => {
  ...
  const roleKey = "octopus.pageAgent.role";
  const memoryKey = "octopus.pageAgent.memoryMode";
  ...
  function notice(text) {
    let el = document.getElementById("octopus-bookmarklet-relay-status");
    if (!el) {
      el = document.createElement("div");
      el.id = "octopus-bookmarklet-relay-status";
      el.style.cssText = [
        "position:fixed",
        "right:16px",
        "bottom:16px",
    ...
    el.textContent = text;
    window.clearTimeout(el.__octopusTimer);
  ...
  function mountAssistant() {
    ...
    panel.id = "octopus-page-agent-panel";
    panel.style.cssText = [
      "position:fixed",
      "left:50%",
      "bottom:28px",
    ...
    status.textContent = "准备就绪，当前网页已连接 Echo";
    ...
    form.addEventListener("submit", (event) => {
      ...
      status.textContent = "已发送到 Echo 对话";
      openInEchoAI(task, {
  ...
  async function runDomAction(action, params = {}) {
    ...
    if (action === "extract" || action === "aria") {
      ...
      const pageAgent = window.__octopusPageAgent?.snapshot?.() || null;
      return {
        ok: true,
        url: location.href,
  ...
  function poll() {
    ...
    const callback = `__octopusBookmarkletPoll_${Date.now()}_${Math.random().toString(36).slice(2)}`;
