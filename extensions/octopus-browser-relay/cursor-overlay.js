(() => {
  ...
  globalThis.__OCTOPUS_CURSOR_OVERLAY_STORE__?.destroy?.();

  const HOST_ID = "octopus-agent-cursor-overlay-host";
  const store = new Map([
    [
      "cursor",
      {
    ...
    shadow.innerHTML = `
      <style>
        ...
        #cursor[data-phase="click"] #pulse {
          animation: octopus-cursor-pulse 520ms ease-out;
        }
        ...
        @keyframes octopus-cursor-pulse {
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type !== "octopus.cursorOverlay") return false;
  ...
  try {
    chrome.runtime.sendMessage(
      { type: "octopus.cursorOverlayReady" },
  ...
  globalThis.__OCTOPUS_CURSOR_OVERLAY_STORE__ = {
