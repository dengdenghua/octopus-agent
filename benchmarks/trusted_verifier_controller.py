...
def _load_sibling_contract() -> Any:
    ...
    expected = os.environ.get("OCTOPUS_CANDIDATE_WORKER_CONTRACT_SHA256", "").strip()
    try:
    ...
    spec = importlib.util.spec_from_file_location(
        "_octopus_trusted_verifier_contract", contract_path
    )
    if spec is None:
...
def evaluate_path_boundary(
    ...
    *,
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="octopus-path-challenge-") as temporary:
...
def _validate_workspace(
    ...
    *,
    ...
    allowed = {
        ".octopus-eval/run-tests",
        ".octopus-eval/run-tests.cmd",
        module_name,
        "pyproject.toml",
        "tests/.gitkeep",
        test_name,
