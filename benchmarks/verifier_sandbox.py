...
_START_MARKER = "__OCTOPUS_HIDDEN_VERIFIER_STARTED_v1__"
BROWSER_UNAVAILABLE_EXIT = 77
HARDENED_RUNNER_ENV = "OCTOPUS_HARDENED_VERIFIER_RUNNER"
HARDENED_RUNNER_CONTRACT: dict[str, Any] = {
    "schema": "octopus.hardened_verifier_runner.v2",
    "required": {
...
_TRUSTED_CONTROLLER_WRAPPERS: dict[tuple[str, str], str] = {
    (
        ...
        "8ffab414d86da41557572cfde220ad749634ff0ff88c79c69b39b03d8756cb0f",
    ): "coding.concurrent-cache",
    (
        "verify_path_boundary.py",
        "bb7497f639d976a93c7bcabd69889183ba5e7ab5b886826ff9ed9c823f40e8c6",
...
code = compile(source, "<octopus-hidden-verifier>", "exec")
source_workspace, shadow_workspace, *verifier_args = sys.argv[1:]
print("__OCTOPUS_HIDDEN_VERIFIER_STARTED_v1__", flush=True)
shutil.copytree(source_workspace, shadow_workspace, symlinks=True)
os.chdir(shadow_workspace)
sys.argv = ["<octopus-hidden-verifier>", *verifier_args]
...
try:
    ...
    connection.close()
checks["inherited_secret"] = os.environ.get("OCTOPUS_SANDBOX_PROBE_SECRET")
...
class _PermissionProbeSandbox:
    ...
    def diagnostics(self) -> dict[str, Any]:
        return {
            "schema": "octopus.hidden_verifier_permission_diagnostic.v1",
            "ownership": "evaluator",
            ...
            "environment_keys": sorted(_minimal_env(Path("/tmp/octopus-provenance")).keys()),
...
def _execute_permission_probe(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="octopus-hidden-verifier-") as temporary:
...
def _probe_permission_boundaries(backend: _PermissionProbeSandbox) -> None:
    with tempfile.TemporaryDirectory(prefix="octopus-sandbox-probe-") as temporary:
