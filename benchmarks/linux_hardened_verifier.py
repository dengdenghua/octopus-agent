...
ATTESTATION_SCHEMA = "octopus.hardened_verifier_runner.v2"
CANDIDATE_WORKER_CONTRACT_SCHEMA = "octopus.candidate_worker_contract.v1"
CANDIDATE_API_ISOLATION_SCHEMA = "octopus.candidate_api_process.v1"
RUNNER_BACKEND = "linux-bubblewrap-cgroup-v2"
WORKER_RPC = "u32be-canonical-json-v1"
RUNNER_READY_KIND = "runner_ready"

CONFIG_ENV = "OCTOPUS_HARDENED_VERIFIER_RUNNER"
WORKER_CONFIG_ENV = "OCTOPUS_CANDIDATE_WORKER_CONFIG"
WORKER_CONFIG_SHA256_ENV = "OCTOPUS_CANDIDATE_WORKER_CONFIG_SHA256"
WORKER_CLI_ENV = "OCTOPUS_CANDIDATE_WORKER_CLI_JSON"
WORKER_CONTRACT_SHA256_ENV = "OCTOPUS_CANDIDATE_WORKER_CONTRACT_SHA256"

_RUN_NAME_RE = re.compile(r"^run-[0-9a-f]{32}$")
_NONCE_RE = re.compile(r"^[A-Za-z0-9_-]{24,128}$")
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
_START_MARKER = b"__OCTOPUS_HARDENED_SANDBOX_READY_v2__"
_PROBE_SECRET_KEY = "OCTOPUS_SANDBOX_PROBE_SECRET"
...
def _validate_runtime_mountpoints(root: Path) -> None:
    ...
    worker_target = root / "octopus-trusted" / "trusted_verifier_worker.py"
    try:
...
class _ScratchLease:
    ...
    def __enter__(self) -> _ScratchLease:
        ...
        lock_path = self.root / ".octopus-hardened-verifier.lock"
    ...
    def _recover_stale(self) -> None:
        for child in self.root.iterdir():
            if child.name == ".octopus-hardened-verifier.lock":
                continue
...
probe = {
    ...
    "snapshot_write_denied": denied(lambda: Path("/workspace/.octopus-probe-write").write_text("x")),
    ...
    "host_secret_scrubbed": os.environ.get("OCTOPUS_SANDBOX_PROBE_SECRET") is None,
...
print("__OCTOPUS_HARDENED_SANDBOX_READY_v2__" + json.dumps(probe, sort_keys=True, separators=(",", ":")), flush=True)
...
sys.argv = ["<octopus-hidden-verifier>", *[part.replace("/workspace", "/work/workspace") for part in verifier_args]]
namespace = {"__name__": "__main__", "__file__": "<octopus-hidden-verifier>"}
exec(compile(source, "<octopus-hidden-verifier>", "exec"), namespace, namespace)
""".lstrip()
...
probe = {
    ...
    "outside_write_denied": denied(lambda: Path(outside_write).write_text("escaped")),
    "snapshot_write_denied": denied(lambda: Path("/workspace/.octopus-probe-write").write_text("x")),
    "challenge_write_denied": denied(lambda: Path("/challenge/.octopus-probe-write").write_text("x")),
    "loopback_network_denied": denied(lambda: socket.create_connection(("127.0.0.1", int(loopback_port)), .2)),
    "external_network_denied": denied(lambda: socket.create_connection(("1.1.1.1", 53), .2)),
    "host_secret_scrubbed": os.environ.get("OCTOPUS_SANDBOX_PROBE_SECRET") is None,
    "controller_pid_hidden": not Path("/proc", controller_pid).exists(),
...
def _worker_bubblewrap_command(
    ...
    *,
    ...
    command = [
        ...
        "/octopus-trusted/trusted_verifier_worker.py",
        "--bind",
        str(work),
        "/work",
        "--seccomp",
    ...
    def run_trusted_controller(
        ...
        *,
        ...
        for label, path, expected in (
            ...
            _assert_digest(path, expected, label)
        evidence = {
            "schema": "octopus.trusted_verifier_controller_run.v2",
            "authorization": True,
            "case_id": case_id,
    ...
    def run_hidden_verifier(
        ...
        *,
        ...
        with _ScratchLease(
            ...
            original_write = workspace_path / ".octopus-original-write-probe"
            ...
            for label, path, expected in (
                ...
                _assert_digest(path, expected, label)
            evidence = {
                "schema": "octopus.hardened_verifier_run.v2",
                "authorization": True,
                "git_sha": self.attestation.git_sha,
                "config_sha256": self.attestation.config_sha256,
                "runtime_pre": runtime_pre.to_dict(),
...
def _load_trusted_supervisor(
    ...
    try:
        ...
        module_name = f"_octopus_attested_worker_{attestation.worker_sha256[:16]}"
...
def worker_cli(argv: Sequence[str]) -> int:
    ...
    parser = InfrastructureParser(
        prog="octopus-hardened-verifier-worker",
        allow_abbrev=False,
    )
        ...
        with _ScratchLease(
            ...
            try:
                ...
                supervisor_thread = threading.Thread(
                    target=supervise,
                    name="octopus-trusted-verifier-supervisor",
                    daemon=True,
                )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments and arguments[0] == "validate":
            ...
            parser = InfrastructureParser(
                prog="octopus-hardened-verifier-validate",
                allow_abbrev=False,
            )
