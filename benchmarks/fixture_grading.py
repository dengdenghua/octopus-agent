...
def _python_test_runner_content(python_executable: str) -> tuple[str, str]:
    if os.name == "nt":
        path = ".octopus-eval/run-tests.cmd"
        content = (
            "@echo off\r\n"
            ...
            "set PYTHONPATH=\r\n"
            "set PYTHONHOME=\r\n"
            'if "%~1"=="--octopus-preflight" goto preflight\r\n'
        ...
        return path, content
    path = ".octopus-eval/run-tests"
    content = (
        "#!/bin/sh\n"
        "export PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1\n"
        "unset PYTHONPATH PYTHONHOME\n"
        'if [ "$#" -eq 1 ] && [ "$1" = "--octopus-preflight" ]; then\n'
...
def _write_python_test_runner(
    ...
    *,
    ...
    runner_dir = workspace / ".octopus-eval"
...
def _preflight_python_test_runner(workspace: Path, provenance: dict[str, str]) -> None:
    ...
    if os.name == "nt":
        ...
        command = [comspec, "/d", "/s", "/c", f'""{runner}" --octopus-preflight"']
    else:
        command = [str(runner), "--octopus-preflight"]
    try:
        completed = subprocess.run(
            command,
            cwd=workspace,
