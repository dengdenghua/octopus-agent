...
| **P3** | 前端 `PanelManifest`；拆 `workspace-sidebar` / `[thread_id]/page.tsx` | ✅ 契约层 + 消费原语 + 真实页面接入 + **首个行为保持拆分**：`TaskCollaboratorControl`（364 行）从 `[thread_id]/page.tsx`（4041→3655 行）抽出为独立模块，137 测试文件/1118 测试全绿；`workspace-sidebar` 拆分待做 |
