...
本地 `docker build -t octopus-agent .` 产生的隐式 `latest` 只是开发标签，不是发布身份。
...
# 分别引用 ${OCTOPUS_LOCAL_AUTH_JWT_SECRET}/${OCTOPUS_ADMIN_PASSWORD_HASH}，
...
Compose 中 `octopus-agent:latest` 仅命名当前本地 build；不会被发布到 GHCR，也不得作为
远程部署依据。该基线的 Redis 镜像固定了明确版本与多架构 manifest digest。

Agent 容器自动读带密码的 `OCTOPUS_HEARTS_REDIS_URL`。跨机或多副本生产需改接托管 Redis / Sentinel / Cluster，并为 `/data` 配置 RWX 共享存储或外部状态后端；`RedisCoordinator` 只负责协调，不能把本地文件状态自动变成共享状态。
...
```bash
# 1. 装代码（从 PyPI 或源码）
# PyPI 发行名是 octopus-agent-runtime；安装后的命令仍是 octopus-agent。
# 不要安装同名的 octopus-agent 发行包，它属于无关的第三方项目。
...
sudo -u octopus /opt/octopus-agent/.venv/bin/pip install \
    "octopus-agent-runtime[serve,local-auth,anthropic]"

# 2. 放生产配置并生成认证材料（缺失/弱值会 fail closed）
sudo cp deploy/systemd-config.yaml /etc/octopus/config.yaml
sudo cp deploy/systemd.env.example /etc/octopus/octopus.env
# 把下面两个生成结果分别写入 octopus.env 对应的空值；bcrypt hash 必须保留单引号
printf 'Octopus!9%s\n' "$(openssl rand -base64 48)"
/opt/octopus-agent/.venv/bin/python -c \
...
sudoedit /etc/octopus/octopus.env
...
`octopus-resources` 命名卷，确保管理员安装的 skills/agents 不随容器重建丢失。包含：
...
**备份**：备份 `./data`，并用
`docker run --rm -v octopus-resources:/source -v "$PWD":/backup alpine tar -czf /backup/octopus-resources.tgz -C /source .`
...
- [ ] 入口控制器 namespace 已加 `octopus-agent.io/ingress-access=true`；Redis 入站只允许 Agent Pod；按实际 provider/DNS 设计过 Egress
