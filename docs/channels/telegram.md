...
通过 Telegram Bot API 将 Octopus-Agent 接入 Telegram，实现与用户在私聊和群组中的智能对话。
...
2. 按提示输入机器人名称（显示名）和用户名（必须以 `bot` 结尾，如 `my_octopus_bot`）
...
使用 Webhook 模式时，Octopus-Agent 启动后会自动调用 Telegram 的 `setWebhook` 接口注册回调地址。
...
A: Octopus-Agent 默认使用 MarkdownV2 格式发送消息。如需切换，可在配置中设置 `parse_mode: HTML` 或 `parse_mode: Markdown`。

## 相关链接

- [Telegram Bot API 官方文档](https://core.telegram.org/bots/api)
- [BotFather 使用指南](https://core.telegram.org/bots/features#botfather)
- [Octopus-Agent 渠道配置文档](https://docs.octopus-agent.dev/channels/telegram)
