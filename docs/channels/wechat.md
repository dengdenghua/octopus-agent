...
通过微信公众号/微信客服接口将 Octopus-Agent 接入微信生态，实现与用户在微信公众号和微信客服中的智能对话。
...
微信会向该 URL 发送 GET 验证请求（包含 `signature`、`timestamp`、`nonce`、`echostr` 参数），Octopus-Agent 会自动使用 Token 完成签名验证并返回 echostr。
...
### Q: 如何发送图文消息？
A: Octopus-Agent 会自动将 AI 的结构化输出转换为微信图文消息格式。如需自定义，可在配置中指定 `news_template`。注意微信图文消息有条数限制（最多 8 条）。
...
- [Octopus-Agent 渠道配置文档](https://docs.octopus-agent.dev/channels/wechat)
