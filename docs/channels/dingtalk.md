...
通过钉钉开放平台的自建机器人将 Octopus-Agent 接入钉钉，支持在单聊和群聊中与用户进行智能对话。
...
钉钉会向该 URL 发送验证请求，Octopus-Agent 会自动完成签名验证。
...
### Q: 如何发送钉钉互动卡片消息？
A: Octopus-Agent 会自动将 AI 的结构化输出转换为钉钉互动卡片。如需自定义卡片模板，可在配置中指定 `card_template_id`。
...
- [Octopus-Agent 渠道配置文档](https://docs.octopus-agent.dev/channels/dingtalk)
