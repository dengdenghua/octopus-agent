...
octopus-agent 已有 `desktop_operator_arm` —— 用 pyautogui / 屏幕截图 + 鼠标键盘
...
**A. 走纯 Octopus-agent 自己的"computer_use_loop"路线**——用 vision 截图
   + pyautogui 等价物在 Android 上实现。
   优点：架构统一。
   缺点：包大（需打包完整 vision 框架）、性能差（截图每帧 2MB+）、
        反爬弱（WebView 默认暴露 webdriver）、不能装 Chrome 扩展。

**B. Fork 一个 Android 端独立 App**（如基于 Octopus Mobile 改造）做本地单 Agent。
   优点：用户体验好、APK 小、运行快。
   缺点：与 Octopus-agent Runtime 隔离，**没有跨端编排能力**，
        没法享受多 Arm 协作 / 自进化 / 预算治理。

**C. 新建"Tentacle"器官**（物理触手），与现有 Arms（逻辑腕）解耦，
   通过 WebSocket + JSON-RPC 2.0 让触手能接入 Runtime。
   优点：保留 Octopus Mobile 现有 30 个 BaseTool 的成熟实现，复用 Octopus-agent
...
2. **不破坏桌面端**：所有新增走 `runtime/tentacle/` + Octopus Mobile 端改造。`runtime/execution/arms/presets.py` 的 `make_desktop_operator_arm` **一字不改**，
...
> LangChain4j 在 Octopus Mobile 中的**真实作用**只有 600 行 Kotlin 能覆盖：
...
│         │                                                │
│         ▼                                                │
│  动作层（30 BaseTool，跟现在 Octopus Mobile 一模一样）          │
...
### 正面影响
- Octopus Mobile 改造版可享受 Octopus-agent 全栈能力（多 Arm、自进化、预算）
- 桌面端 90% 代码完全不动
- 30 个移动技能 100% 复用 Octopus Mobile 现有实现
- 用户体验：本地模式跟现在 Octopus Mobile 一模一样（无学习成本）
...
- INV-3：设备锁（device/lock）由 Runtime 统一管理，Octopus Mobile 端只执行
