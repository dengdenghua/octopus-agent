...
// OCTOPUS_LINUX_ARCH selects the target slice; it defaults to the host arch.
const LINUX_ARCH = process.env.OCTOPUS_LINUX_ARCH || process.arch;
...
if (!PROFILE) {
  throw new Error(
    `unsupported OCTOPUS_LINUX_ARCH: ${LINUX_ARCH} (expected x64 or arm64)`,
  );
}
...
function prepare() {
  ...
  fs.writeFileSync(
    path.join(outputRoot, "octopus-codex-bundle.json"),
    `${JSON.stringify(
      {
