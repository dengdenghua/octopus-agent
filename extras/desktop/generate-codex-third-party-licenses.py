...
def _generate_reports(*, source: Path, cargo_about: Path, destination: Path) -> dict[str, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="octopus-codex-licenses-") as raw_temp:
...
def _generate_ripgrep_report(*, source: Path, cargo_about: Path, destination: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="octopus-ripgrep-licenses-") as raw_temp:
        temp_root = Path(raw_temp)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="octopus-codex-license-output-") as raw_output:
